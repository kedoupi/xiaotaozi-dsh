...
- 本机 `pnpm smoke:sandbox` 未成功启动：固定 3081 被另一 checkout 的 sandbox 占用。`scripts/smoke-sandbox.mjs#assertSandboxPortEmpty` 在任何写入/构建前安全拒绝，未杀该进程、未碰 official；clean-runner 成功证据仍待 CI。按约束不能抢占或停止不属于本 checkout 的 sandbox。
