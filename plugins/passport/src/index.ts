...
import { explainAuthError } from "./auth/explain.ts";
...
import { registerPassportRpc } from "./rpc.ts";
...
class PassportAuthController implements AuthController {
  ...
  private async completeDevice(provider: "qwen" | "kimi", attempt: Awaited<ReturnType<DeviceFlowManager["start"]>>): Promise<void> {
    try {
      ...
      if (!(error instanceof Error && (error.message === "login cancelled" || error.message === "已取消登录"))) {
        this.lastError.set(provider, explainAuthError(error));
      }
    }
  }
...
export function apply(ctx: Context, config: Config): void {
  ...
  const onWarn = (message: string): void => {
    ctx.logger.warn(`dsh-passport: ${message}`);
  };
  ...
  registerPassportRpc(ctx, new PassportAuthController(flows, devices, authChanged, usageFetchers, catalogs, providers), providers);
}
