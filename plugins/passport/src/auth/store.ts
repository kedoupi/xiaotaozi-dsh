/**
 * On-disk OAuth session store at `~/.dsh/plugins/passport/auth.json`.
 *
 ...
 * @returns `~/.dsh/plugins/passport/auth.json`.
 */
...
function assertSessionShape(provider: ProviderId, value: unknown): asserts value is StoredSession {
  if (typeof value !== 'object' || value === null) {
    throw new Error(`passport auth store: entry "${provider}" is not an object; fix or delete the store file`)
  }
  ...
  if (typeof entry.accessToken !== 'string' || entry.accessToken.length === 0
    ...
    throw new Error(
      `passport auth store: entry "${provider}" is missing accessToken/refreshToken/expiresAt; fix or delete the store file`,
    )
  }
}
...
function parseStore(text: string, path: string): SessionMap {
  ...
  try {
    ...
    throw new Error(`passport auth store at ${path} is not valid JSON; fix or delete the file`)
  }
  if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
    throw new Error(`passport auth store at ${path} must be a JSON object keyed by provider; fix or delete the file`)
  }
