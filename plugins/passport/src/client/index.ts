...
import type { PassportKey } from "./locales.ts";
import { css } from "./styles.ts";

declare module "@deepseek-ai/dsh-client-ui-slots" {
  interface LocaleNamespaceMap {
    "settings.passport": PassportKey;
  }
}

const NS = "settings.passport";

export const inject = ["slots", "connection", "locale"];

function ensureStyles(): () => void {
  const existing = document.querySelector('style[data-plugin-css="dsh-passport"]');
  if (existing !== null) return () => {};
  const node = document.createElement("style");
  node.dataset.pluginCss = "dsh-passport";
...
export function apply(ctx: ClientContext): void {
  ctx.effect(() => ensureStyles(), "dsh-passport css");
  ctx.effect(() => ctx.locale.register(NS, { zh, en }), "dsh-passport copy");
  ...
  ctx.effect(() => hideOfficialModels(t("nav")), "dsh-passport hide official Models");
