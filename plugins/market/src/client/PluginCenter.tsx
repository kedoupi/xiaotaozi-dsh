...
export function PluginCenter({
  ...
  const statusText = [
    ...
    refreshError,
    refreshing ? t("refreshing") : announcement || queueText,
  ]
    .filter(Boolean)
    .join(" ");
  ...
  const list = (tab: CenterTab): JSX.Element => {
    const entries = snapshot
      ? searchCatalog(snapshot.entries, location.query, location.tag)
      : [];
    const installed =
      snapshot?.installedPlugins.filter((row) =>
        row.name
          .toLocaleLowerCase()
          .includes(location.query.toLocaleLowerCase()),
      ) ?? [];
    return (
      <div className="dsh-market-list">
        <h2 ref={headingRef} tabIndex={-1}>
          {t(tab === "installed" ? "tabInstalled" : "tabDiscover")}
        </h2>
        {tab === "installed" && (
          <section aria-label={t("builtIn")}>
            <h3>{t("builtIn")}</h3>
            <div className="dsh-market-capabilities">
              {CAPABILITIES.map((row) => (
                <button
                  ...
                  className="dsh-market-capability"
                  ref={(node) => rememberCard(row.id, node)}
                  onClick={() =>
                    openDetail({ kind: "capability", id: row.id }, row.id)
                  }
                >
                  <Icon name={entryIconName(row.id, "plugin")} size={22} />
                  <span>{t(row.name)}</span>
                  <span>{t(row.summary)}</span>
                  <span>
        ...
        {catalogError ? (
          <p className="dsh-market-error">{t("loadError")}</p>
        ) : !snapshot ? (
          <p>{t("loading")}</p>
        ) : (
          <>
            <div className="dsh-market-discovery">
              <div className="dsh-market-search-field">
                ...
                <input
                  ...
                  onChange={(event) =>
                    center.navigate({ ...location, query: event.target.value })
                  }
                />
              </div>
              {tab === "discover" && (
                <div className="dsh-market-tags">
                  {["", ...tagsOf(snapshot.entries)].map((tag) => (
                    <button
                      key={tag}
                      type="button"
                      className="dsh-market-tag"
                      aria-pressed={location.tag === tag}
                      data-active={location.tag === tag}
                      onClick={() =>
                        center.navigate({
                          ...location,
                          tag: location.tag === tag ? "" : tag,
                        })
                      }
                    >
                      {tag || t("allTags")}
            ...
            {(tab === "installed" ? installed.length : entries.length) === 0 ? (
              <div className="dsh-market-empty">
                <span>
                  {t(
                    tab === "installed" &&
                      snapshot.installedPlugins.length === 0
                      ? "installedEmpty"
                      : "empty",
                  )}
                </span>
                <button
                  ...
                  onClick={() =>
                    center.navigate({ ...location, query: "", tag: "" })
                  }
                >
                  {t("resetFilters")}
                </button>
              ...
              <div className="dsh-market-grid">
                {tab === "installed"
                  ? installed.map((entry) => (
                      <article key={entry.id} className="dsh-market-card">
                        <button
                          type="button"
                          id={`dsh-market-card-${entry.id}`}
                          ref={(node) => rememberCard(entry.id, node)}
                          className="dsh-market-card-open"
                          aria-label={`${t("openDetails")}: ${entry.name}`}
                          onClick={() =>
                            openDetail(
                              { kind: "installed", id: entry.id },
                              entry.id,
                            )
                          }
                        >
                          <span className="dsh-market-card-name">
                            {entry.name}
                          </span>
                          <span>
                              ...
                              : (snapshot.sources.find(
                                  (source) =>
                                    ...
                                    snapshot.entries.find(
                                      (row) => row.id === entry.catalogEntryId,
                                    )?.sourceId,
                                )?.label ?? t("installed"))}
                          </span>
                          <span>{t(presentationFor(entry).label)}</span>
                          <span>
                            {t(
                              RUNTIME_LABELS[
                                runtimeStateFor(entry.packageName, inventory)
                              ],
                            )}
                          </span>
                        </button>
                  ...
                  : entries.map((entry) => (
                      <Card
                        ...
                        onOpen={() =>
                          openDetail(
                            { kind: "catalog", id: entry.id },
                            entry.id,
                          )
                        }
                        onQueue={(row, action) =>
                          onQueue(row.id, row.sourceId, action)
                        }
                      />
                    ))}
              </div>
  ...
  return (
    <section
      ...
      className="dsh-market-center"
      aria-labelledby="dsh-plugin-center-title"
    >
      <header className="dsh-market-center-head">
        <span className="dsh-market-center-mark">
          <img src={PORTRAIT} alt="" width={36} height={36} />
        </span>
        <div className="dsh-market-center-titles">
          <h1 id="dsh-plugin-center-title" tabIndex={-1}>
            {t("nav")}
          </h1>
          <p>{t("subtitle")}</p>
        </div>
        <button
          type="button"
          className="dsh-market-center-close"
          aria-label={t("close")}
          onClick={onClose}
        >
          <Icon name="close" />
      ...
      >
        {(["installed", "discover"] as const).map((tab) => (
          <button
            ...
            id={`dsh-market-tab-${tab}`}
            className="dsh-market-tab"
            aria-selected={location.tab === tab}
            aria-controls={`dsh-market-panel-${tab}`}
            tabIndex={location.tab === tab ? 0 : -1}
      ...
      <div ref={scrollRef} className="dsh-market-center-scroll">
        {embedded ? (
          statusText && <p className="dsh-market-page-status">{statusText}</p>
        ) : (
          <div
        ...
        {(location.tab === "installed" || target?.kind === "installed") && (
          <div>
            <button
              type="button"
              className="dsh-market-secondary"
              data-retry="inventory"
        ...
        {catalogError && (
          <button
            ...
            className="dsh-market-secondary"
            data-retry="catalog"
            disabled={busyId !== undefined || refreshing}
            onClick={retryLoads}
          >
            {t("retry")}
          </button>
        ...
        {(["installed", "discover"] as const).map((tab) => (
          <div
            ...
            hidden={location.tab !== tab}
          >
            {location.tab === tab &&
              (capability ? (
                <div className="dsh-market-capability-detail">
                  <button
                    type="button"
                    className="dsh-market-back"
                    onClick={back}
                  >
                    <Icon name="arrowLeft" />
                    {t("back")}
                  </button>
                  <h2 ref={capabilityHeading} tabIndex={-1}>
                    {t(capability.name)}
                  </h2>
                  {configuration}
                </div>
                ...
                <PluginDetail
                  ...
                  target={target}
                  snapshot={snapshot}
                  presentation={presentationFor(target.entry)}
                  runtimeState={
                    target.kind === "installed"
                      ? runtimeStateFor(target.entry.packageName, inventory)
                      : "unknown"
                  }
                  configuration={embedded ? configuration : undefined}
    ...
    <article className="dsh-market-card">
      ...
      {showGet && (
        <button
          ...
          onClick={() => {
            if (!disabled && !blocked) onQueue(entry, action);
          }}
        >
          {presentation.retryable
            ? t("retry")
