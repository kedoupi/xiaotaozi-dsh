  ...
  while (true) {
    ...
    if (packageJson?.name === "@deepseek-ai/dsh") {
      if (packageJson.version !== PINNED_DSH_VERSION) {
        throw new Error(
          `current DSH Host is ${String(packageJson.version)}, expected ${PINNED_DSH_VERSION}`,
        );
      }
      const relativeBin = dshBin(packageJson);
      if (relativeBin === undefined)
        throw new Error("current DSH Host package has no dsh bin entry");
      ...
      if (packageBin !== realEntry)
        throw new Error(
          "current DSH Host entry does not match its package dsh bin",
        );
...
export function spawnDshPluginMutate(
  ...
  if (typeof spec !== "string" || spec === "") {
    return Promise.resolve({
      ok: false,
      error:
        action === "install" ? "missing install spec" : "missing package name",
    });
  }
  ...
  try {
    ...
    return Promise.resolve({
      ok: false,
      error: "pinned DSH runtime unavailable",
    });
  }
  return runPinnedPlugin(launch, args, env, runtime).then((first) => {
    if (first.ok && !(action === "install" && first.buildDenied))
      return { ok: true };
    // Subprocess output is diagnostic, never authority to grant script trust.
    // No automatic retry: this seam cannot prove an exact preauthorized Git identity.
    return failedMutate(
      first.buildDenied ? `allowBuilds blocked\n${first.error}` : first.error,
      first.output,
    );
  });
}
...
function failedMutate(fallback: string, output: string): PluginMutateResult {
  const combined = [output, fallback]
...
function collectStream(
  ...
  stream.on("data", (chunk: string) => {
    const text = (chunks[0] ?? "") + chunk;
    // Classify before truncation; the retained suffix also joins split markers.
    if (classifyMutateError(text) === "allow-builds-blocked") onBuildDenied();
    chunks.splice(0, chunks.length, text.slice(-32_000));
  });
}
...
function runPinnedPlugin(
  ...
  return new Promise((resolve) => {
    ...
    const finish = (result: Omit<SpawnOutcome, "buildDenied">): void => {
      if (settled) return;
      settled = true;
      clearTimeout(timer);
      resolve({ ...result, buildDenied });
    };
    const onBuildDenied = (): void => {
      buildDenied = true;
    };
    collectStream(child.stdout, stdout, onBuildDenied);
    collectStream(child.stderr, stderr, onBuildDenied);
    let timedOut = false;
    timer = setTimeout(() => {
      timedOut = true;
      killTree();
    ...
    child.on("close", (code) => {
      ...
      if (timedOut) {
        finish({
          ok: false,
          error: `${args.includes("add") ? "install" : "remove"} timed out`,
          output,
        });
        return;
      }
      if (code === 0) {
        finish({ ok: true, error: "", output });
        return;
      }
      finish({
        ok: false,
