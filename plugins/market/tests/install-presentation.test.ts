...
describe("install presentation", () => {
  it("distinguishes known partial repair from unverified dependency inventory", () => {
    expect(
      installPresentation({ ...base, installationState: "partial" }),
    ).toMatchObject({
      status: "partial",
    ...
    ).toMatchObject({ status: "partial", action: "remove" });
    expect(
      installPresentation({
        ...base,
        installed: true,
        installationUnverified: true,
      }),
    ).toMatchObject({
      status: "unverified",
