...
const entries: CatalogEntry[] = [
  {
    ...
    name: "Alpha Tools",
    version: "1.0.0",
    summary: "Team utilities",
    tags: ["Collaboration"],
    kind: "plugin",
    sourceId: "official",
    installed: true,
    installSpec: "alpha-tools",
  },
  {
    id: "beta",
    name: "Beta Memory",
    ...
    sourceId: "official",
    installed: false,
    installSpec: "github:example/beta-memory",
  },
  {
    id: "gamma",
    name: "Gamma Memory",
    version: "1.0.0",
    summary: "Durable recall",
    tags: ["Memory"],
    kind: "plugin",
    sourceId: "official",
    installed: true,
    installSpec: "gamma-memory",
  },
];
const sources = [
  {
    id: "official",
    label: "Xiaotaozi catalog",
...
const installedFor = (rows) =>
  rows
    .filter((row) => row.installed)
    .map((row) => ({
      id: `installed:${row.id}-package`,
      packageName: `${row.id}-package`,
      name: row.name,
      installSpec: row.installSpec,
      source: "catalog",
...
async function renderCenter(options = {}, face = {}) {
  const center = face.center ?? createPluginCenterOpen();
  center.open();
  let renderer;
  await act(async () => {
    renderer = create(
      React.createElement(PluginCenter, {
        center,
        ctx: { slots: defaultSlots, get: () => undefined },
        t,
...
async function renderDiscover(options = {}, face = {}) {
  ...
  await act(async () =>
    renderer.root
      .findByProps({ id: "dsh-market-tab-discover" })
      .props.onClick(),
  );
  return renderer;
}
...
function response(payload) {
  return {
    json: async () => ({
      ...
      ...(payload.entries
        ? {
            entries: payload.entries.map((row) => ({
              ...row,
              installationState:
                row.installationState ??
                (row.installed ? "installed" : "absent"),
            })),
            installedPlugins: installedFor(payload.entries),
          }
        : {}),
    }),
  };
...
describe("PluginCenter initial visit", () => {
  ...
  it("shows all four built-ins while catalog loads, with installed selected", async () => {
    ...
    expect(
      view.root.findAll((node) => node.props["data-capability"] !== undefined),
    ).toHaveLength(4);
    const tabs = view.root.findAllByProps({ role: "tab" });
    expect(tabs).toHaveLength(2);
    expect(tabs[0].props["aria-selected"]).toBe(true);
    expect(
      view.root.findAllByProps({ "data-capability": "models" }),
    ).toHaveLength(1);
...
describe("market discovery controls", () => {
        ...
        json: async () =>
          String(input).endsWith("/intents")
            ...
            : {
                ...
                sources,
                installedPlugins: installedFor(entries),
                entries,
              },
      })),
    );
  ...
  it("filters catalog entries as the user types", async () => {
    ...
    await act(async () =>
      search.props.onChange({ target: { value: "alpha" } }),
    );
  ...
  it("keeps labelled search and category controls before the discovery grid", async () => {
    ...
    expect(
      pressed.find((node) => textOf(node) === en.allTags)?.props[
        "aria-pressed"
      ],
    ).toBe(true);
  ...
  it("explains empty matches and resets every discovery control", async () => {
    const renderer = await renderDiscover();
    const pressedButton = (label: string) =>
      renderer.root
        .findAll(
          (node) =>
            node.type === "button" && node.props["aria-pressed"] !== undefined,
    ...
    expect(
      renderer.root.findByProps({ id: "dsh-market-search" }).props.value,
    ).toBe("");
  ...
  it("keeps catalog cards concise with a labelled source and sibling action", async () => {
    const renderer = await renderDiscover();
    const beta = cards(renderer).find((card) =>
      textOf(card).includes("Beta Memory"),
    );
    const alpha = cards(renderer).find((card) =>
      textOf(card).includes("Alpha Tools"),
    );
  ...
  it("keeps every market action outside other buttons and labels structural SVG controls", async () => {
    ...
    const assertActions = () => {
      ...
      for (const svg of view.root.findAllByType("svg"))
        expect(svg.props["aria-hidden"]).toBe("true");
      expect(textOf(view.root)).not.toMatch(/[×‹]/);
    };
    assertActions();
    await act(async () =>
      view.root.findByProps({ "data-capability": "models" }).props.onClick(),
    );
    assertActions();
    await act(async () =>
      view.root.findByProps({ id: "dsh-market-tab-discover" }).props.onClick(),
    );
    assertActions();
    await act(async () =>
      view.root
        .findByProps({ "aria-label": `${en.openDetails}: Alpha Tools` })
        .props.onClick(),
    );
    assertActions();
    await act(async () =>
      view.root
        .findByProps({ className: "dsh-market-install" })
        .props.onClick(),
    );
    assertActions();
  });
  ...
  it("identifies npm install details without Git or official claims", async () => {
    ...
    await act(async () =>
      alpha.findByProps({ className: "dsh-market-card-open" }).props.onClick(),
    );

    const detail = renderer.root.findByProps({
      className: "dsh-market-detail",
    });
  ...
  it("moves exact install and transparent risk information into detail", async () => {
    ...
    await act(async () =>
      beta.findByProps({ className: "dsh-market-card-open" }).props.onClick(),
    );
    ...
    expect(detailText).toContain(
      "dsh plugin --profile web add github:example/beta-memory",
    );
    ...
    expect(
      renderer.root.findByProps({ className: "dsh-market-detail-name" }).props
        .tabIndex,
    ).toBe(-1);
  });
});
...
describe("install lifecycle presentation", () => {
  ...
  it("shows every loaded pending intent as queued without claiming active host progress", async () => {
    ...
    expect(
      beta.findByProps({ className: "dsh-market-get" }).props.disabled,
    ).toBe(true);
    const announcer = renderer.root.findByProps({
      className: "dsh-market-announcer",
    });
  ...
  it("shows installing, transient completion, then durable installed truth", async () => {
    ...
    const renderer = await renderDiscover();
    const installButton = cards(renderer)
      .find((card) => textOf(card).includes("Beta Memory"))
      .findByProps({ className: "dsh-market-get" });

    await act(async () => installButton.props.onClick());

    expect(
      textOf(
        cards(renderer).find((card) => textOf(card).includes("Beta Memory")),
      ),
    ).toContain(en.installing);
    expect(
      textOf(renderer.root.findByProps({ className: "dsh-market-announcer" })),
    ).toContain(`Beta Memory: ${en.installing}`);
    ...
    expect(
      textOf(renderer.root.findByProps({ className: "dsh-market-detail" })),
    ).toContain(en.installCompleted);
    expect(
      textOf(renderer.root.findByProps({ className: "dsh-market-announcer" })),
    ).toContain(`Beta Memory: ${en.installCompleted}`);
  ...
  it("retains an acknowledged cleanup error without claiming completion or retrying", async () => {
    const cleanupError =
      "Plugin install completed, but intent cleanup failed. Do not retry the plugin mutation until the state file is repaired.";
    let catalogLoads = 0;
    vi.stubGlobal(
      "fetch",
      vi.fn(async (input, init) => {
        ...
        if (String(input).endsWith("/intents"))
          return response({ ok: true, intents: [] });
        catalogLoads += 1;
        return response({
          ok: true,
          allowThirdPartySources: false,
    ...
    expect(textOf(beta)).not.toContain(en.installCompleted);
    expect(beta.findAllByProps({ className: "dsh-market-get" })).toHaveLength(
      0,
    );
    expect(
      textOf(renderer.root.findByProps({ className: "dsh-market-announcer" })),
    ...
    expect(
      textOf(renderer.root.findByProps({ className: "dsh-market-announcer" })),
    ).not.toContain(`Beta Memory: ${en.installCompleted}`);
  });
  ...
  it("keeps failure and retry ownership on the failed entry", async () => {
    ...
    const betaFailed = cards(renderer).find((card) =>
      textOf(card).includes("Beta Memory"),
    );
    const alpha = cards(renderer).find((card) =>
      textOf(card).includes("Alpha Tools"),
    ...
    expect(
      textOf(betaFailed.findByProps({ className: "dsh-market-get" })),
    ).toBe(en.retry);
    expect(textOf(alpha)).not.toContain(en.installFailed);
    expect(
      textOf(betaFailed.findByProps({ className: "dsh-market-error" })),
    ).toBe("disk full");
    expect(
      textOf(renderer.root.findByProps({ className: "dsh-market-announcer" })),
    ).toContain("Beta Memory: disk full");

    await act(async () =>
      betaFailed.findByProps({ className: "dsh-market-get" }).props.onClick(),
    );
    ...
    ).toContain(en.retryingInstall);
    expect(
      textOf(
        cards(renderer).find((card) => textOf(card).includes("Alpha Tools")),
      ),
    ).not.toContain(en.retryingInstall);
    expect(
      textOf(renderer.root.findByProps({ className: "dsh-market-announcer" })),
    ).toContain(`Beta Memory: ${en.retryingInstall}`);
  ...
  it("retries the owned failed action even if a later snapshot changes installed truth", async () => {
    ...
    await act(async () =>
      renderer.root
        .findByProps({ className: "dsh-market-confirm-remove" })
        .props.onClick(),
    );
    await act(async () =>
      renderer.root
    ...
    expect(
      textOf(detail.findByProps({ className: "dsh-market-install" })),
    ).toContain(en.retry);

    await act(async () =>
      detail.findByProps({ className: "dsh-market-install" }).props.onClick(),
    );
  ...
  it("requires an accessible confirmation before removing", async () => {
    ...
    const backdrop = renderer.root.findByProps({
      className: "dsh-market-confirm-overlay",
    });
    for (const handler of ["onClick", "onMouseDown", "onPointerDown"])
      expect(backdrop.props[handler]).toBeUndefined();
    expect(confirmation.props["aria-modal"]).toBe("true");
    expect(confirmation.props["aria-labelledby"]).toBe(
      "dsh-market-remove-title",
    );
    expect(confirmation.props["aria-describedby"]).toBe(
      "dsh-market-remove-description",
    );
    ...
    await act(async () =>
      confirmation
        .find((node) =>
          node.props.className?.includes("dsh-market-confirm-cancel"),
        )
        .props.onClick(),
    );
    expect(posts).toBe(0);
  ...
  it("contains confirmation keys and focuses Installed heading after confirm", async () => {
    ...
    const dialog = {
      ...
      contains: (node) =>
        node === dialog || node === cancel || node === confirm,
    ...
    await act(async () => {
      host = create(
        React.createElement(PluginCenterHost, {
          center,
          ctx: {},
          t,
          renderSlot: () => null,
    ...
    const renderer = await renderDiscover(
      {
        createNodeMock: (element) => {
          ...
          if (
            element.type === "h2" &&
            element.props.children === en.tabInstalled
          )
            return installedHeading;
  ...
  it("announces truthful remove progress and completion", async () => {
    ...
    expect(
      textOf(renderer.root.findByProps({ className: "dsh-market-detail" })),
    ).toContain(en.removing);
    expect(
      textOf(renderer.root.findByProps({ className: "dsh-market-announcer" })),
    ).toContain(`Alpha Tools: ${en.removing}`);
    ...
    await act(async () =>
      removal.resolve({
        ...
        installedPlugins: [],
        entries: entries.map((entry) =>
          entry.id === "alpha" ? { ...entry, installed: false } : entry,
        ),
      }),
    );

    expect(
      renderer.root.findAllByProps({ className: "dsh-market-detail" }),
    ).toHaveLength(0);
    expect(
      renderer.root.findByProps({ id: "dsh-market-tab-installed" }).props[
        "aria-selected"
      ],
    ).toBe(true);
    expect(
      textOf(renderer.root.findByProps({ className: "dsh-market-announcer" })),
    ).toContain(`Alpha Tools: ${en.removeCompleted}`);
  });
});
...
describe("PluginCenter failure boundaries and navigation", () => {
  ...
  const open = async (view, name) =>
    act(async () =>
      view.root
        .findByProps({ "aria-label": `${en.openDetails}: ${name}` })
        .props.onClick(),
    );
  ...
  it("keeps built-ins viewable after catalog failure and distinguishes retry/loading/empty", async () => {
    ...
    serve((input) =>
      String(input).endsWith("/catalog")
        ? {
            json: async () =>
              fail
                ? { ok: false, error: "profile unreadable" }
                : { ...catalog, entries: [], installedPlugins: [] },
          }
        : undefined,
    );
    const view = await renderCenter();
    ...
    await act(async () =>
      view.root.findByProps({ className: "dsh-market-back" }).props.onClick(),
    );
    fail = false;
    await act(async () =>
      view.root.findByProps({ "data-retry": "catalog" }).props.onClick(),
    );
  ...
  it("uses live slot winners and falls back after the selected winner abdicates", async () => {
    serve();
    let winners = [{ options: { key: "models" } }];
    let version = 0;
    let notify;
    const slots = {
      getVersion: () => version,
    ...
    const renderSlot = vi.fn((_name, _owner, options) =>
      winners.length
        ? React.createElement("p", { role: "status" }, "Models contribution")
        : options.fallback,
    );
    const view = await renderCenter(
      {},
      { ctx: { slots, get: () => undefined }, renderSlot },
    );
    expect(
      textOf(view.root.findByProps({ "data-capability": "models" })),
    ).not.toContain(en.unavailable);
    await act(async () =>
      view.root.findByProps({ "data-capability": "models" }).props.onClick(),
    );
    expect(renderSlot).toHaveBeenLastCalledWith(
      ...
      {},
      expect.objectContaining({ entryKey: "models" }),
    );
    expect(
      view.root.findAllByProps({ className: "dsh-market-announcer" }),
    ).toHaveLength(0);
  ...
  it("loads inventory lazily, reports failure and retries without blocking removal", async () => {
    ...
    const view = await renderCenter(
      {},
      {
        ...
        ctx: {
          slots: defaultSlots,
          get: () => ({ pluginInventory: { list } }),
        },
      },
    );
    expect(list).not.toHaveBeenCalled();
    await act(async () =>
      view.root.findByProps({ id: "dsh-market-tab-installed" }).props.onClick(),
    );
    expect(list).toHaveBeenCalledOnce();
    await open(view, "Alpha Tools");
    expect(textOf(view.root)).toContain(en.runtimeUnknown);
    expect(
      view.root.findByProps({ className: "dsh-market-install" }).props.disabled,
    ).toBe(false);
    ...
    list.mockResolvedValue({
      ...
      value: {
        entries: [
          { moduleName: "alpha-package", enabled: true, fiberPhase: "active" },
        ],
      },
    });
    await act(async () =>
      view.root.findByProps({ "data-retry": "inventory" }).props.onClick(),
    );
  ...
  it("selects and focuses keyboard tabs, keeps both panels, and ignores composition", async () => {
    serve();
    const focus = { installed: vi.fn(), discover: vi.fn() };
    const view = await renderCenter({
      createNodeMock: (element) =>
        element.props.role === "tab"
          ? {
              focus:
                focus[
                  element.props.id.endsWith("installed")
                    ? "installed"
                    : "discover"
                ],
            }
    ...
    const key = async (tab, value, composing = false) =>
      act(async () =>
        view.root.findByProps({ id: `dsh-market-tab-${tab}` }).props.onKeyDown({
          key: value,
    ...
    expect(
      view.root.findByProps({ id: "dsh-market-tab-installed" }).props[
        "aria-selected"
      ],
    ).toBe(true);
    for (const [from, keyName, to] of [
      ["installed", "End", "discover"],
      ["discover", "Home", "installed"],
      ["installed", "ArrowLeft", "discover"],
      ["discover", "ArrowRight", "installed"],
    ]) {
      await key(from, keyName);
      expect(
        view.root.findByProps({ id: `dsh-market-tab-${to}` }).props.tabIndex,
      ).toBe(0);
      expect(
        view.root.findByProps({ id: `dsh-market-panel-${from}` }).props.hidden,
      ).toBe(true);
  ...
  it("restores Back query, tag, actual scroll and original card focus without scrolling", async () => {
    ...
    const view = await renderDiscover({
      createNodeMock: (element) =>
        element.props.className === "dsh-market-center-scroll"
          ? scroller
          : element.props.id === "dsh-market-card-beta"
            ? { focus }
            : null,
    });
    await act(async () =>
      view.root
        .findByProps({ id: "dsh-market-search" })
        .props.onChange({ target: { value: "beta" } }),
    );
    await act(async () =>
      view.root
        .findAllByProps({ className: "dsh-market-tag" })
        .find((n) => textOf(n) === "Memory")
        .props.onClick(),
    );
    ...
    expect(view.root.findByProps({ id: "dsh-market-search" }).props.value).toBe(
      "beta",
    );
    expect(
      view.root
        .findAllByProps({ className: "dsh-market-tag" })
        .find((n) => textOf(n) === "Memory").props["aria-pressed"],
    ).toBe(true);
    await act(async () =>
      view.root.findByProps({ id: "dsh-market-tab-installed" }).props.onClick(),
    );
  ...
  it("restores the current Discover filters and scroll after direct card installation", async () => {
    ...
    serve((_input, init) =>
      init?.method === "POST" ? pending.promise : undefined,
    );
    const scroller = { scrollTop: 0 };
    const view = await renderDiscover({
      createNodeMock: (element) =>
    ...
    await act(async () =>
      view.root
        .findByProps({ id: "dsh-market-search" })
        .props.onChange({ target: { value: "memory" } }),
    );
    await act(async () =>
      view.root
        .findAllByProps({ className: "dsh-market-tag" })
    ...
    await act(async () =>
      view.root
        .findByProps({ "aria-label": `${en.install}: Beta Memory` })
        .props.onClick(),
    );
    await act(async () =>
      view.root
    ...
    expect(
      textOf(view.root.findByProps({ className: "dsh-market-detail" })),
    ).toContain("Beta Memory");
  ...
  it.each([false, true])(
    "ignores an older retry catalog after completed install (applied warning=%s)",
    async (appliedWarning) => {
      ...
      serve((input, init) => {
        ...
        if (String(input).endsWith("/intents"))
          return response(
            ++intentReads === 1
              ? { ok: false, error: "intents unreadable" }
              : { ok: true, intents: [] },
          );
        if (String(input).endsWith("/catalog")) {
          ...
          return catalogReads === 2
            ...
            : response(catalogReads === 1 ? catalog : installedCatalog);
        }
      });
      const view = await renderDiscover();
      expect(
        view.root.findByProps({ "aria-label": `${en.install}: Beta Memory` })
          .props.disabled,
      ).toBe(true);
      await act(async () =>
        view.root.findByProps({ "data-retry": "intents" }).props.onClick(),
      );
      await open(view, "Beta Memory");
      expect(
        view.root.findByProps({ className: "dsh-market-install" }).props
          .disabled,
      ).toBe(false);
      await act(async () =>
        view.root
      ...
      const assertInstalled = () => {
        ...
        expect(
          textOf(view.root.findByProps({ className: "dsh-market-install" })),
        ).toBe(en.remove);
        expect(
          view.root.findByProps({ className: "dsh-market-install" }).props
            .disabled,
        ...
        expect(
          view.root.findAllByProps({ "data-retry": "applied" }),
        ).toHaveLength(0);
      };
      assertInstalled();
      ...
      expect(
        view.root.findAllByProps({
          "aria-label": `${en.install}: Beta Memory`,
        }),
      ).toHaveLength(0);
      expect(
        view.root.findAllByProps({ "aria-label": `${en.retry}: Beta Memory` }),
      ).toHaveLength(0);
      expect(
        fetch.mock.calls.filter(([, init]) => init?.method === "POST"),
      ).toHaveLength(1);
  ...
  it("blocks mutations on intents failure but preserves browsing and supplied configuration until retry succeeds", async () => {
    ...
    const slots = {
      ...defaultSlots,
      entriesOfSlot: () => [{ options: { key: "alpha-package" } }],
    };
    const view = await renderCenter(
      {},
      {
        ctx: { slots, get: () => undefined },
        renderSlot: () =>
          React.createElement("button", {}, "Plugin-owned configuration"),
      },
    );
    await open(view, "Alpha Tools");
    ...
    expect(
      view.root.findAllByProps({ className: "dsh-market-detail" }),
    ).toHaveLength(0);
  });

  it.each([false, true])(
    "preserves applied warning/no snapshot through failed reads and refresh only (cleanup=%s)",
    async (cleanup) => {
      let reads = 0;
      let repaired = false;
      const warning = cleanup
        ? "Plugin installed but cleanup failed"
        : "Plugin installed but profile projection failed";
      serve((input, init) => {
        if (init?.method === "POST")
          return response({
            ok: false,
        ...
        if (String(input).endsWith("/catalog")) {
          reads++;
          if (reads > 1 && !repaired)
            return response({ ok: false, error: "profile still unreadable" });
          return response({
            ...catalog,
            entries: repaired
              ? entries.map((e) =>
      ...
      await act(async () =>
        view.root.findByProps({ "data-retry": "applied" }).props.onClick(),
      );
      expect(textOf(view.root)).toContain(warning);
      repaired = true;
      await act(async () =>
  ...
  it("retains the original failed mutation and action alongside projection diagnostics", async () => {
    ...
    serve((_input, init) => {
      if (init?.method === "POST") {
        ...
        return response({
          ok: false,
          error: "disk full; profile projection unavailable",
          intents: [],
        });
      }
    });
    ...
    expect(textOf(view.root)).toContain(
      "disk full; profile projection unavailable",
    );
    expect(textOf(view.root)).toContain(en.installFailed);
    expect(textOf(view.root)).not.toContain(en.installCompleted);
    await act(async () =>
      view.root
        .findByProps({ "aria-label": `${en.retry}: Beta Memory` })
        .props.onClick(),
    );
    expect(posted).toEqual([
      { entryId: "beta", sourceId: "official", action: "install" },
      { entryId: "beta", sourceId: "official", action: "install" },
    ]);
  });
  ...
  it("removes external installs only with named confirmation and profile id", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (_input, init) => ({
        json: async () =>
          init?.method === "POST"
            ? { ...catalog, intents: [], installedPlugins: [] }
            : String(_input).endsWith("/intents")
              ? { ok: true, intents: [] }
              : { ...catalog, installedPlugins: [extra] },
      })),
    );
    const view = await renderCenter();
    await open(view, "@example/extra");
    ...
    expect(textOf(view.root.findByProps({ role: "alertdialog" }))).toContain(
      "@example/extra",
    );
    await act(async () =>
      view.root
    ...
    expect(
      ...
      entryId: "installed:%40example%2Fextra",
      sourceId: "profile",
      action: "remove",
    });
    expect(textOf(view.root)).toContain(
      `@example/extra: ${en.removeCompleted}`,
    );
  });
  ...
  it("guards same-tick/global busy across details and ignores settlement after unmount", async () => {
    ...
    const center = createPluginCenterOpen();
    const view = await renderDiscover({}, { center });
    const install = view.root.findByProps({
      "aria-label": `${en.install}: Beta Memory`,
    });
    await act(async () => {
    ...
    await act(async () =>
      pending.resolve(
        response({
          ...catalog,
          intents: [],
          entries: entries.map((e) => ({ ...e, installed: true })),
        }),
      ),
    );
...
describe("PluginCenter additional lifecycle regressions", () => {
  afterEach(() => vi.unstubAllGlobals());
  it("keeps a partial catalog repair visible while retaining explicit installed-key removal", async () => {
    const partial = {
      ...entries[1],
    ...
    const installed = {
      id: "installed:beta-package",
      packageName: "beta-package",
      name: partial.name,
      installSpec: partial.installSpec,
      source: "catalog",
      catalogEntryId: "beta",
    };
    vi.stubGlobal(
    ...
    const view = await renderDiscover();
    expect(textOf(cards(view)[0])).toContain("Partially installed");
    await act(async () =>
      view.root
        .findByProps({ "aria-label": `${en.openDetails}: Beta Memory` })
        .props.onClick(),
    );
    expect(
      textOf(view.root.findByProps({ className: "dsh-market-install" })),
  ...
  it.each(["beta-alias", "beta-package"])(
    "offers ambiguous Installed rows for explicit %s removal",
    async (packageName) => {
      const ambiguous = {
        ...entries[1],
        packageName: "beta-package",
      ...
      let installed = ["beta-alias", "beta-package"].map((key) => ({
        id: `installed:${key}`,
        packageName: key,
        name: ambiguous.name,
        source: "catalog",
      ...
      vi.stubGlobal(
        ...
        vi.fn(async (input, init) => {
          if (init?.method === "POST") {
            ...
            posts.push(body);
            installed = installed.filter((row) => row.id !== body.entryId);
          }
          return {
            json: async () =>
              String(input).endsWith("/intents") && init?.method !== "POST"
                ? { ok: true, intents: [] }
                : {
                    ok: true,
                    intents: [],
      ...
      for (const key of ["beta-alias", "beta-package"])
        expect(
          view.root.findByProps({ id: `dsh-market-card-installed:${key}` }),
        ).toBeDefined();
      await act(async () =>
        view.root
          .findByProps({ id: `dsh-market-card-installed:${packageName}` })
          .props.onClick(),
      );
      expect(
        textOf(view.root.findByProps({ className: "dsh-market-install" })),
      ...
      expect(posts).toEqual([
        {
          entryId: `installed:${packageName}`,
          sourceId: "profile",
          action: "remove",
        },
      ]);
      expect(installed.map((row) => row.packageName)).toEqual([
        packageName === "beta-alias" ? "beta-package" : "beta-alias",
      ]);
    },
  );

  it("keeps confirmed actual-key removal after a partial catalog repair fails", async () => {
    const partial = {
      ...entries[1],
      packageName: "beta-package",
      installed: false,
    ...
    const posts: Array<{ entryId: string; sourceId: string; action: string }> =
      [];
    const warning = "repair failed; retained dependency";
    let removed = false;
    vi.stubGlobal(
      "fetch",
      vi.fn(async (input, init) => {
        if (init?.method === "POST") {
          ...
          if (body.action === "install")
            return response({ ok: false, error: warning, intents: [] });
          removed = true;
        }
        return {
          json: async () =>
            String(input).endsWith("/intents") && init?.method !== "POST"
              ...
              : {
                  ...
                  entries: [
                    {
                      ...partial,
                      installationState: removed ? "absent" : "partial",
                    },
                  ],
                  installedPlugins: removed ? [] : [installed],
                },
        };
      }),
    );
    ...
    expect(posts).toEqual([
      { entryId: "beta", sourceId: partial.sourceId, action: "install" },
    ]);
    expect(
      textOf(view.root.findByProps({ className: "dsh-market-install" })),
    ).toBe(en.retry);
    ...
    expect(textOf(view.root)).toContain(warning);
    const remove = view.root.findByProps({ className: "dsh-market-install" });
    expect(textOf(remove)).toBe(en.remove);
    ...
    expect(posts).toEqual([
      { entryId: "beta", sourceId: partial.sourceId, action: "install" },
      {
        entryId: "installed:beta-package",
        sourceId: "profile",
        action: "remove",
      },
    ]);
    expect(
      posts.filter(
        (post) => post.sourceId === "profile" && post.action === "install",
      ),
    ).toEqual([]);
  });

  it("does not settle acknowledged removal from dependency absence while its bundle is partial", async () => {
    let changed = false;
    let retired = false;
    const warning = "remove acknowledged, bundle remains";
    vi.stubGlobal(
      "fetch",
      vi.fn(async (input, init) => {
        if (init?.method === "POST") {
        ...
        return {
          json: async () => ({
            ...
            sources,
            entries: [alpha],
            installedPlugins: changed ? [] : installedFor([alpha]),
          }),
        };
      }),
    );
    ...
    expect(view.root.findAllByProps({ "data-retry": "applied" })).toHaveLength(
      1,
    );
    expect(textOf(view.root)).toContain(warning);
    expect(textOf(view.root)).not.toContain(en.removeCompleted);
    retired = true;
    await act(async () =>
      view.root.findByProps({ "data-retry": "applied" }).props.onClick(),
    );
  ...
  it.each(["partial", "unknown", "wrong-target"])(
    "does not settle acknowledged %s installation until fresh same-target coherent state",
    async (unverified) => {
      let reads = 0;
      let repaired = false;
      const warning =
        "acknowledged but verification failed; rollback not attempted";
      vi.stubGlobal(
        "fetch",
        vi.fn(async (input, init) => {
          if (init?.method === "POST")
          ...
          if (String(input).endsWith("/intents"))
            return response({ ok: true, intents: [] });
          reads++;
          const state = repaired
            ? "installed"
            : reads === 1
              ? "absent"
              : unverified === "unknown"
                ? undefined
                : unverified === "wrong-target"
                  ? "installed"
                  : "partial";
          const beta = {
            ...entries[1],
          ...
          const row = {
            id: "installed:beta-package",
            packageName:
              repaired || unverified !== "wrong-target"
                ? "beta-package"
                : "other-package",
            name: beta.name,
            installSpec: beta.installSpec,
            source: "catalog",
            catalogEntryId: "beta",
          };
          return {
      ...
      const view = await renderDiscover();
      await act(async () =>
        view.root.findByProps({ className: "dsh-market-get" }).props.onClick(),
      );
      expect(reads).toBe(2);
      expect(textOf(view.root)).toContain(warning);
      expect(textOf(view.root)).not.toContain(en.installCompleted);
      expect(
        view.root.findByProps({ "data-retry": "applied" }).props.disabled,
      ).toBe(false);
      for (const button of view.root.findAllByProps({
  ...
  it.each([
    [true, "active", "running"],
    [true, "loading", "runtimeLoading"],
    [true, "failed", "runtimeError"],
    [false, "active", "disabled"],
    [true, null, "runtimeUnknown"],
  ])(
    "pairs installed truth with visible runtime words (enabled=%s, phase=%s)",
    async (enabled, fiberPhase, label) => {
      vi.stubGlobal(
        "fetch",
        vi.fn(async (input) =>
          response(
            ...
            get: () => ({
              pluginInventory: {
                list: async () => ({
                  ...
                  value: {
                    entries: [
                      { moduleName: "alpha-package", enabled, fiberPhase },
                    ],
                  },
                }),
              },
      ...
      const card = view.root.findByProps({
        "aria-label": `${en.openDetails}: Alpha Tools`,
      });
      expect(textOf(card)).toContain(en.installed);
      expect(textOf(card)).toContain(en[label]);
      await act(async () => card.props.onClick());
      expect(
        textOf(view.root.findByProps({ className: "dsh-market-meta" })),
      ).toContain(en[label]);
      expect(
        view.root.findByProps({ className: "dsh-market-install" }).props
          .disabled,
  ...
  it("focuses the newly installed card when returning from successful installed detail", async () => {
          ...
          init?.method === "POST"
            ? {
                ...
                entries: entries.map((row) =>
                  row.id === "beta" ? { ...row, installed: true } : row,
                ),
              }
            : String(input).endsWith("/intents")
              ? { ok: true, intents: [] }
    ...
    const view = await renderDiscover({
      createNodeMock: (element) =>
        element.props.id === "dsh-market-card-installed:beta-package"
          ? { focus }
          : null,
    });
    await act(async () =>
...
describe("PluginDetail confirmation mutation lock", () => {
  ...
  it("does not submit a confirmation opened before mutation controls become disabled", async () => {
    ...
    let view;
    await act(async () => {
      view = create(React.createElement(PluginDetail, props));
    });
    mounted.push(view);
    await act(async () =>
      view.root
    ...
    await act(async () =>
      view.update(
        React.createElement(PluginDetail, { ...props, mutationDisabled: true }),
      ),
    );
    await act(async () =>
      view.root
    ...
    await act(async () =>
      view.update(React.createElement(PluginDetail, props)),
    );
    await act(async () =>
      view.root
        .findByProps({ className: "dsh-market-install" })
    ...
    expect(onQueue).toHaveBeenCalledExactlyOnceWith(
      "installed:alpha-package",
      "profile",
      "remove",
    );
  });
