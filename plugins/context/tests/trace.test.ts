...
import { pluginTrace, pluginTraceEnabled, sanitizeTraceArg, TRACE_NS } from "../src/host/trace";
...
it("pluginTrace writes one namespaced line when enabled", () => {
  const chunks: string[] = [];
  pluginTrace("mounted projections=timeline,headers", { DSH_PLUGIN_TRACE: "1" }, (chunk) => {
    chunks.push(chunk);
  });
  expect(chunks).toEqual([`[${TRACE_NS}] mounted projections=timeline,headers\n`]);
});
