 ...
 * results.
 *
 * Installation (bundle): `dsh plugin add github:kedoupi/xiaotaozi-dsh#path:plugins/agent-teams`
...
import { displayCaptainName, normalizeRoster, usageIdentityText, type RosterMember } from './names.ts'
...
export interface Config {
  ...
  /** Display name for the captain. Routing and mailboxes still use `captain`. */
  captainName?: string
  /** Preset roster. Non-empty: added when a team is created. */
...
export const Config: z<Config> = z.object({
  ...
  captainName: z.string().default('张老板'),
...
function usageSectionText(toolNames: string, captainName: string, members: readonly RosterMember[]): string {
  return `${usageIdentityText(captainName, members)}

When the user asks to run something with AgentTeams (e.g. "use AgentTeams to do X"), or an activation message from the /agent-teams slash command arrives, follow this protocol:
...
export function apply(ctx: Context, config: Config): void {
  ...
  if (config.slashCommand ?? true) {
    ctx.inject(['commands'], (commandCtx) => {
      registerAgentTeamsCommand(commandCtx, captainName)
    })
    installAgentTeamsGestureBoundary(ctx, captainName)
  }
