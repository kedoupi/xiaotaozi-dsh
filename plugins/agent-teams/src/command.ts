 ...
 */
export function buildActivationDirective(goal: string, captainName?: string): string {
  const goalLine = goal === ''
    ? 'The goal was not given — ask the user what the team should accomplish.'
    : `Goal: ${goal}`
  const captain = displayCaptainName(captainName)
  return [
    `The user invoked the \`/agent-teams\` command. Activate the AgentTeams protocol from your instructions now: you are ${captain}, the captain of a multi-agent team.`,
    goalLine,
  ].join('\n')
}
 ...
 */
export function registerAgentTeamsCommand(ctx: Context, captainName?: string): void {
  ctx.effect(() => ctx.commands.register({
    name: AGENT_TEAMS_COMMAND,
    description: `run a goal with a multi-agent team (you become ${displayCaptainName(captainName)})`,
    ...
    handler(invocation: CommandInvocation): CommandResult {
      ...
      return {
        kind: 'success',
        text: `AgentTeams activated — ${displayCaptainName(captainName)} will assemble a team for: ${goal}`,
      }
    },
 ...
 */
export function installAgentTeamsGestureBoundary(ctx: Context, captainName?: string): void {
  ctx.on('agent/pre-step', async (
    ...
    const activation = createUserMessage({
      content: [{ type: 'text', text: buildActivationDirective(goal, captainName) }],
      source: {
