...
import { defineTool, type ToolRunContext } from './tool-define.ts'
import { displayCaptainName, type RosterMember } from './names.ts'
...
export interface ToolsConfig {
  ...
  maxMembers: number
  /** Display name for the captain. Routing still uses `captain`. */
  captainName: string
  /** Preset roster added on team create when non-empty. */
...
export function registerAgentTeamsTools(ctx: Context, config: ToolsConfig): void {
  const tools = (ctx as Context & { tools: { register(tool: unknown): void } }).tools
  ...
  tools.register(defineTool({
    ...
    description: 'Create a new AgentTeams team: you (the calling agent) become the captain. A captain leads one team at a time. If the plugin has a preset member roster, those members are added immediately; otherwise add them afterwards with agent_teams_add_member.',
    parameters: {
      ...
      return withTeamLock(captainLockKey(stateRoot, captain.id), async () => {
        ...
        return withTeamLock(teamLockKey(stateRoot, teamId), async () => {
          ...
          const state: TeamState = {
            ...
            captainName: displayCaptainName(config.captainName),
            createdAt: Date.now(),
            members: [],
            tasks: [],
            taskSeq: 0,
          ...
          for (const preset of config.members) {
            const memberName = preset.name.trim()
          ...
          if (state.members.length > 0) await writeTeam(stateRoot, state)
