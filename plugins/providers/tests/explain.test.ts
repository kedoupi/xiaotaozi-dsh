...
import { explainAuthError, explainHostError, isLoginCancelled } from "../src/auth/explain.ts";
...
describe("explainAuthError", () => {
  it("hides english and status codes", () => {
    ...
    expect(isLoginCancelled(new Error("已取消登录"))).toBe(true);
    expect(isLoginCancelled(new Error("login cancelled"))).toBe(true);
    expect(isLoginCancelled(new Error("授权没有完成，请再试一次"))).toBe(false);
  });
