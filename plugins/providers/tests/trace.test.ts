...
it("pluginTrace writes one namespaced line when enabled", () => {
  const chunks: string[] = [];
  pluginTrace("rpc login provider=codex start", { DSH_PLUGIN_TRACE: "1" }, (chunk) => {
    chunks.push(chunk);
  });
  expect(chunks).toEqual([`[${TRACE_NS}] rpc login provider=codex start\n`]);
});

it("sanitizeTraceArg redacts secret-shaped keys", () => {
  expect(sanitizeTraceArg({ authorization: "Bearer abc" })).toContain("<redacted>");
  expect(sanitizeTraceArg({ authorization: "Bearer abc" })).not.toContain("Bearer abc");
});
