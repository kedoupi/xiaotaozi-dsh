...
- **对话只显示勾选过的模型。** 勾选立刻生效。
- **可选智能选择。** 插件中心 → 已安装 → 模型有「智能选择」开关（默认关）。开启后，对话里不再选手动模型；每个人类提问由系统从已勾选且已授权的模型里选定。没有可选项时会拦住发送，并提醒去插件中心勾选。工具后续 Step 固定。余额/授权类硬失败会排除该服务商并改投其他已勾选服务商一次；同账号的其他模型不会当作替补。不做辅助模型分类、在线学习、reasoning effort 路由、耐久路由审计、按会话模式。
