...
export function ModelsList(props: {
    ...
    if (props.models.length === 0)
        return <p className="dshM-hint">{props.t("modelsNone")}</p>;
    ...
    const visibleRest = rest.slice(
        0,
        Math.max(0, MODEL_PREVIEW - selected.length),
    );
            ...
            <label className="dshM-check">
                ...
                <span>
                    ...
                    {model.name === model.id ? null : (
                        <span className="dshM-modelId">{model.id}</span>
                    )}
                </span>
            </label>
        </li>
        ...
        <>
            <div className="dshM-actions">
                <button
                    type="button"
                    className="dshM-btn"
                    disabled={enabled === props.models.length}
                    onClick={() =>
                        void props.onSave(props.models.map((model) => model.id))
                    }
                >
                    {props.t("enableAll")}
                </button>
            ...
            {extra.length > 0 ? (
                <details className="dshM-manual">
                    ...
                    <ul className="dshM-models">{extra.map(renderModel)}</ul>
                </details>
            ) : null}
        </>
    );
    ...
    return (
        <section aria-busy={props.pending || undefined}>
            ...
            {vendor.configured && !props.replacing ? (
                <>
                    <div
                        className="dshM-secret"
                        aria-label={t("apiMaskedLabel")}
                    >
                        <span className="dshM-secretMask">
                            {t("apiMasked")}
                        </span>
                    </div>
                    {locked ? (
                        <p className="dshM-hint" style={{ marginTop: 10 }}>
                            {t("envKeyLocked")}
                        </p>
                    ) : (
                ...
                <>
                    ...
                    <label className="dshM-fieldLabel" htmlFor={keyInputId}>
                        {t("apiTitle")}
                    </label>
