  ...
  return {
    ...
    buildAuthorizeUrl({ redirectUri, state, pkce, nonce }) {
      const params = new URLSearchParams({
        ...
        nonce,
        plan: 'generic',
        // Registered xAI OAuth client referrer from the upstream plugin.
...
export class GrokAdapter extends LlmAdapter {
  ...
  async availableModels(provider: string): Promise<LlmModelInfo[]> {
  ...
  override async listModels(provider: string): Promise<readonly LlmModelInfo[]> {
    return advertisedModels('grok', await this.availableModels(provider))
  }
