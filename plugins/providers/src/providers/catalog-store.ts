 ...
 * `$DSH_HOME/plugins/providers/models.json` — the durable half of each
 ...
 * @returns `$DSH_HOME/plugins/providers/models.json`.
 */
export function modelsFilePath(): string {
  return pluginData('models.json')
}
...
async function readCatalogFile(path: string): Promise<CatalogFile> {
  if (path === modelsFilePath()) await migrateLegacyPluginData()
  let text: string
  try {
