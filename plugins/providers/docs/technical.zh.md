...
- `smart` 时占用宿主 `conversation.input.model`（priority `-1`，渲染 `null`）隐藏选择器；`conversation.input.dock` 以 list `order: 80` 展示空池引导与可选「本轮模型」（居中贴 `--dsh-chat-content-width`，默认可收起，不挡输入）。`manual` 时卸下占用。听 `routing-live`，不要求重启。
...
| FR-ROUTE-UX-5 | dock `<details>` 弱展示 lastSelected；list `order`；内容宽居中 | `SmartUx.tsx`、`smart-ux.ts`、`styles.ts`；无决策则不渲染 |
