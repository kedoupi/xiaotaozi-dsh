...
export async function apply(ctx: unknown, config: Record<string, unknown> = {}) {
  ...
  const controller = config.controller as DiscordHostController | undefined;
  if (controller) {
    return installDiscordRpc(host, controller, config.rpcAuthority);
  }
  ...
  return installOwnedProduction(
    host,
    production,
    () => installDiscordRpc(host, production.controller, config.rpcAuthority),
    'dsh-im: close Discord bot connections',
  );
}

export function createDiscordHostPlugin(config: Record<string, unknown> = {}) {
