...
import { runCompactCommand } from './compact-command.ts';
import {
  isControlCommand,
  runControlCommand,
} from './control-command.ts';
import {
  rememberDirectTargetAndFlush,
...
} from './harness-question.ts';
import { deliverOutboundArtifacts } from './semantic/artifact-delivery.ts';
import {
  ...
  accept(message) {
    ...
    if (normalized.kind === 'direct') {
      void rememberDirectTargetAndFlush(
        ...
        (target, content) => this.#bot.sendText(target, content),
      );
    }
