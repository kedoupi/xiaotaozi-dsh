...
} from './repair-manager.ts';
import {
  GROUP_MESSAGE_PERMISSION_OPERATION,
  GroupMessagePermissionManager,
} from './group-message-permission-manager.ts';
import { REQUIRED_TENANT_SCOPES } from './plugin-controller.ts';
import {
...
export class MultiBotDshFeishuController {
  ...
  async #acceptCredentials(record, result) {
    ...
    try {
      ...
      if (record.createdNew) {
        void sendBindUsageGuide(this.#runtimes.get(botId), {
          channelLabel: '飞书',
    extraCommands: [
      ['/repair', '修复卡片按钮回调'],
      ['/m or /menu', '打开交互卡片菜单'],
