...
import { deriveWeixinBotIdentity, maskWeixinAccountId } from './config-store.ts';
import {
...
function closedError() {
  const error = new Error('dsh-weixin controller is closed');
  error.code = 'controller-closed';
  return error;
}
...
export class WeixinController {
  ...
  async #startRuntime(config, token) {
    ...
    try {
      ...
      await runtime.stop().catch(() => undefined);
      if (error?.code === 'controller-closed') throw error;
