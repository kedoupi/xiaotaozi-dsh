...
import {
  ...
  runPresetCommand,
} from '../src/channels/shared/preset-command.ts';
import { withSessionBindingLock } from '../src/channels/shared/session-binding-lock.ts';
...
test('numeric selection rejects an expired snapshot and requires a new /presetlist', async () => {
  let now = 1_000;
