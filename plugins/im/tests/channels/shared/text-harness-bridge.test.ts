...
import { WhatsappHarnessBridge } from '../../../src/channels/whatsapp/whatsapp-bridge.ts';
...
test('delivers a queued bind usage guide on the first direct message', async () => {
  const fixture = stateFixture({ 'direct:chat-a': 'session-welcome' });
  const sent = [];
  queuePendingGuide(fixture.state, 'queued welcome');
  ...
  await bridge.accept(message('welcome-1', 'hello'));
  await eventually(() => sent.includes('queued welcome'));
  assert.ok(sent.includes('queued welcome'));
});
...
test('all four shared text channels list models locally and advertise all four commands', async () => {
  for (const [name, Bridge] of [
    ...
    assert.match(help, /\/workspace  按列表序号或唯一项目名切换项目/, `${name} project switch`);
    assert.match(help, /\/workspacelist  列出 Web 中已创建的项目/, `${name} project list`);
    assert.match(help, /\/sessionlist  列出当前项目的会话；可带项目序号/, `${name} project sessions`);
    assert.match(help, /\/session Session ID 或当前项目会话序号  将当前聊天绑定到指定会话/, `${name} session selection`);
    assert.doesNotMatch(help, /绝对路径|工作区序号|当前项目序号/, `${name} path help`);
  }
});
