...
import { DiscordBotClient } from '../../../src/channels/discord/discord-runtime.ts';
import { createEditableMessageStream } from '../../../src/channels/shared/editable-message-stream.ts';
import { TextHarnessBridge } from '../../../src/channels/shared/text-harness-bridge.ts';
import { SlackBotClient } from '../../../src/channels/slack/slack-runtime.ts';
import { TelegramBotClient } from '../../../src/channels/telegram/telegram-runtime.ts';
import { WhatsappBotClient } from '../../../src/channels/whatsapp/whatsapp-runtime.ts';
...
test('plain Slack, Telegram, Discord, and WhatsApp receipts retain every split message id', async () => {
  ...
  const whatsapp = new WhatsappBotClient({
    ...
    reserve: (messageId) => remembered.push(messageId),
  });
  ...
  assert.equal(remembered.length, 4);
  assert.match(remembered[0], /^[0-9A-F]{20}$/);
  assert.equal(remembered[1], 'whatsapp-1');
  assert.match(remembered[2], /^[0-9A-F]{20}$/);
  assert.equal(remembered[3], 'whatsapp-2');
});
