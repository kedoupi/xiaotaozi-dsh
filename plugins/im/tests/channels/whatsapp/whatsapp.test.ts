...
import {
  WhatsappConfigStore,
  deriveWhatsappBotId,
} from '../../../src/channels/whatsapp/config-store.ts';
import { WhatsappController } from '../../../src/channels/whatsapp/whatsapp-controller.ts';
import {
  WhatsappRuntime,
  normalizeWhatsappMessage,
} from '../../../src/channels/whatsapp/whatsapp-runtime.ts';
import { createWhatsappWebSession } from '../../../src/channels/whatsapp/whatsapp-web-session.ts';
import {
  WHATSAPP_ENDPOINTS,
  createWhatsappRpcHandler,
} from '../../../src/host/channels/whatsapp/rpc.ts';
...
test('WhatsApp exposes image media through a bounded Baileys download stream', async () => {
  ...
  const cancelled = normalizeWhatsappMessage({
    key: { remoteJid: ACCOUNT_JID, id: 'cancelled-1', fromMe: true },
    message: {
  ...
  const document = normalizeWhatsappMessage({
    key: { remoteJid: ACCOUNT_JID, id: 'document-image-1', fromMe: true },
    message: {
      documentMessage: {
  ...
  const oversized = normalizeWhatsappMessage({
    key: { remoteJid: ACCOUNT_JID, id: 'oversized-1', fromMe: true },
    message: {
...
test('WhatsApp runtime connects a linked device and replies through Harness', async () => {
  ...
  await callbacks.onMessage({
    key: { remoteJid: '16505550999@s.whatsapp.net', id: 'stranger-1', fromMe: false },
    message: { conversation: 'please run tools' },
  });
  assert.equal(askCount, 0);
  await callbacks.onMessage({
    key: { remoteJid: ACCOUNT_JID, id: 'direct-2', fromMe: true },
    message: { conversation: 'hello' },
  });
