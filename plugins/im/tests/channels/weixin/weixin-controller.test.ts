...
import { WeixinController } from '../../../src/channels/weixin/weixin-controller.ts';
...
test('close drains an in-flight start and stops the runtime', async () => {
  const credentials = credentialsFixture();
  const configs = configFixture();
  let releaseStart;
  const gate = new Promise((resolve) => { releaseStart = resolve; });
  let startEntered;
  const started = new Promise((resolve) => { startEntered = resolve; });
  let stopCount = 0;
  const controller = new WeixinController({
    api: {
      beginLogin: async () => ({ qrcode: 'qr', qrcodeUrl: 'https://liteapp.weixin.qq.com/q/test' }),
      pollLogin: async () => ({ status: 'wait' }),
    },
    credentials: credentials.provider,
    configStore: configs.store,
    createRuntime: async () => ({
      status: { ready: false, weixinConnectionState: 'connecting', harnessReachable: false },
      async start() {
        startEntered();
        await gate;
      },
  ...
  credentials.values.set('DSH_WEIXIN_TOKEN_TEST', 'token');
  await configs.store.save({
    botId: 'wx_testbot',
    accountId: 'account@im.bot',
    tokenRef: 'DSH_WEIXIN_TOKEN_TEST',
    ownerUserId: 'owner',
    baseUrl: 'https://ilinkai.weixin.qq.com',
  });
