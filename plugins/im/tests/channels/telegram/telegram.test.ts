...
} from '../../../src/channels/telegram/config-store.ts';
import { TelegramController } from '../../../src/channels/telegram/telegram-controller.ts';
import {
  TelegramApi,
  inspectTelegramToken,
  validTelegramToken,
} from '../../../src/channels/telegram/telegram-api.ts';
...
test('Telegram config and controller store only a credential reference in bot data', async (t) => {
  ...
  assert.match(connectionTests[0], /\/help/);
  assert.match(connectionTests.at(-1), /Harness Telegram/);
  assert.match(connectionTests.at(-1), /123•••/);
