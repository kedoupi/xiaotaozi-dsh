...
} from '../../../src/channels/shared/semantic/artifact.ts';
import { TelegramApi } from '../../../src/channels/telegram/telegram-api.ts';
...
} from '../../../src/channels/telegram/telegram-rich-message.ts';
import { TelegramBotClient } from '../../../src/channels/telegram/telegram-runtime.ts';
...
async function committedArtifact() {
  ...
  onTestFinished(() => releaseOutboundArtifact(artifact));
  return artifact;
}
...
test('shared bridge surfaces a definite final delivery failure without rerunning Prompt or artifacts', async () => {
  for (const safeReplyFails of [false, true]) {
      ...
      assert.doesNotMatch(safeReplies[0], /private|secret/i);
...
test('shared bridge settles each artifact once before containing a definite text failure', async () => {
...
test('Telegram keeps an uncertain Rich receipt and still delivers the registered result file', async () => {
  const artifact = await committedArtifact();
  const sentFiles = [];
  const stream = {
    providerMessageIds: [],
...
test('Telegram Rich text and a result file keep the placeholder and file message ids', async () => {
  const artifact = await committedArtifact();
  const sent = [];
  const edited = [];
  const files = [];
