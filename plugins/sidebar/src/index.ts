 ...
 * loopback-only browser-trust fence. The generic web runtime may serve a LAN
 * trusted host, but this surface exposes filesystem and PTY capabilities and
 * therefore never inherits network-level trusted-host access.
 *
...
import { extractFrameAncestors, probePublicHttpHeaders, UnsafeProbeTargetError } from './browser-probe.ts'
...
export function mediaResponseHeaders(path: string, download = false): Record<string, string> {
  const active = ACTIVE_DOCUMENT_EXTENSIONS.has(extname(path).toLowerCase())
  const headers: Record<string, string> = {
    'content-type': active ? 'application/octet-stream' : mediaTypeForPath(path),
...
/**
 * Resolve a session's authoritative working directory. A renderer-provided
 * cwd is never authority: the session must exist in the host store, and only
 * its header may select a workspace. A known blank session uses the host
 * process cwd until its header is hydrated.
 */
function sessionCwdOf(ctx: Context, sessionId: string): string {
  const session = ctx.sessions.get(sessionId)
  if (session === undefined) {
    throw new SidebarError('not-found', `session "${sessionId}" is not attached`, 404)
  }
...
function buildApi(
  ...
  return {
    ...
    // public http(s) addresses with DNS pinning, per-hop redirect validation
    // and a hard timeout. It never sends a GET body-producing request.
    'browser.probe': async (payload) => {
      ...
      try {
        const response = await probePublicHttpHeaders(parsed, controller.signal)
        const cspValue = response.headers['content-security-policy']
        const csp = Array.isArray(cspValue) ? cspValue[0] ?? null : cspValue ?? null
        const frameAncestors = extractFrameAncestors(csp)
        const xFrameValue = response.headers['x-frame-options']
        const xFrameOptions = Array.isArray(xFrameValue) ? xFrameValue[0] : xFrameValue
        return {
          reachable: true,
          url: response.url,
          status: response.status,
          ...(xFrameOptions !== undefined ? { xFrameOptions } : {}),
...
export function apply(ctx: Context, config?: SidebarConfig): void {
  ...
  const fence = (req: SidebarHttpRequest, requireOrigin = false): boolean => isTrustedApiRequest(req, requireOrigin)
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        // HTML/SVG can execute script when opened as a top-level same-origin
        // document, so the raw route always forces them to download. HTML
        // previews use the separate CSP-sandboxed /sidebar/html route.
        const headers = mediaResponseHeaders(path, url.searchParams.get('download') === '1')
