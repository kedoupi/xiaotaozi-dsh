/**
 * zh/en copy for the sidebar. The copy follows the DSH i18n system: the
 * client apply attaches the locale service (`ctx.locale`, provided by
 ...
 * language is used. The dictionaries are also registered into the DSH
 * locale registry under {@link LOCALE_NS}.
 *
 * {@link attachBetterLocale} remains a no-op-safe hook so an optional
 * override store can still win when attached; this plugin no longer ships
 * extra language dictionaries.
 */
...
export const zh = {
  ...
  mermaidZoomOut: '缩小',
  mermaidZoomReset: '重置',
  mermaidZoomTitle: '图表缩放',
  mermaidZoomDialog: 'Mermaid 图表预览',
  mermaidZoomHint: '滚轮缩放 · 拖拽平移 · Esc 关闭',
  refresh: '刷新',
  refreshUnsavedTitle: '丢弃未保存的编辑？',
  refreshUnsavedConfirm: '文件已在磁盘更新，刷新将丢弃未保存编辑。',
  moveUnsavedConfirm: '此操作会关闭或移动含有未保存编辑的窗格。请取消并保存，或丢弃编辑并重新加载文件，然后重试操作。',
  discardAndReload: '丢弃并重新加载',
  save: '保存',
  saved: '已保存',
  unsaved: '未保存',
  saveFailed: '保存失败',
  ...
  settingsNav: '侧边卡片',
  settingsIntro: '管理侧边工作台里显示的内容和默认行为',
  settingsIdentity: '侧边工作台',
  ...
  settingsOpenToolsDesc: '开启后，模型可以在侧边工作台里打开文件、文件夹和网页（默认关闭）',
...
export const en: Record<keyof typeof zh, string> = {
  ...
  openMention: 'Open {path}',
  openWithMenu: 'Open with',
  openWithSshSuffix: ' (SSH)',
  pinOpenWith: 'Pin to menu',
  unpinOpenWith: 'Unpin',
  manageOpenWithPins: 'Manage pinned apps',
  ...
  mermaidZoomTitle: 'Diagram zoom',
  mermaidZoomDialog: 'Mermaid diagram preview',
  mermaidZoomHint: 'Scroll to zoom · drag to pan · Esc to close',
  refresh: 'Refresh',
  refreshUnsavedTitle: 'Discard unsaved edits?',
  refreshUnsavedConfirm: 'The file changed on disk. Refreshing will discard unsaved edits.',
  moveUnsavedConfirm: 'This action would close or move a pane with unsaved edits. Cancel and save, or discard edits and reload the file, then retry the action.',
  ...
  gitConflictHint: 'Resolve manually',
  ...
  settingsIntro: 'Manage what the side workbench shows and how it behaves',
  settingsIdentity: 'Side workbench',
  ...
  settingsOpenToolsDesc: 'When enabled, the model can open files, folders, and web pages in the side workbench (off by default)',
...
export function t(key: CopyKey, params?: Record<string, string | number>): string {
  // 1. Optional override store (when attached) wins when it has a
  //    translation for this (ns, key). Otherwise the zh/en chain runs.
...
export function isZh(): boolean {
  ...
  // en, so isZh() returns false to route selectors to the non-zh branch.
  const dshActive = localeService?.getSnapshot().active ?? ''
