...
- **Git 图谱**：会话上的分支胶囊，打开提交图（SVG 泳道、合并曲线、分支标签）。右侧 Git 负责 status 和 diff。
...
会话上的分支胶囊（空白会话跟在模式胶囊后，紧凑会话在 composer 边）：搜索并切换本地分支，打开提交图（SVG 泳道、合并曲线、分支标签）。工作区 status / diff 在右侧 Git。点菜单外部或 Escape 会收起。切换是工作区级 `git switch`。没有遥测。

## 归档

**插件中心 → 已安装 → 小桃子功能 → 管理归档会话。** 会话 ⋯ 菜单也可进入。搜索或按项目筛选平面会话列表，预览最近对话，单条或批量恢复，并通过明确确认永久删除。只读写 `$DSH_HOME`。
