...
import { XTZ_UI_BOARD_PREFIX } from "../names.ts";
...
export function registerBoardRoutes(webServer: WebServer, service: BoardService): () => void {
  const disposers = [
    ...
    webServer.register({
      kind: "exact",
      path: `${XTZ_UI_BOARD_PREFIX}/tasks`,
      handler: async (req, res) => {
        if (req.method === "POST") {
