...
export interface RuntimeCredentials {
  ...
  unset?(payload: { ref: string }): Promise<RuntimeWire<unknown>>;
}
export type RuntimeFormSnapshot = {
  ...
  available: boolean;
  writable: boolean;
  error: "invalid" | "saveFailed" | "credentialRemoveUnavailable" | undefined;
...
type SaveCapture = {
  ...
  replacement: boolean;
  removal: boolean;
  credentialIntent: number;
  ref: string;
};

export const createRuntimeForm: CreateRuntimeForm = (
  namespace,
  scope,
  credentials,
) => {
  const definitions: Readonly<Record<string, "number" | "text">> =
    RUNTIME_FIELDS[namespace];
  const search = namespace === "web-search-deepseek";
  const drafts = new Map<string, Draft>();
  const listeners = new Set<() => void>();
  let replacement: string | undefined;
  let removalRef: string | undefined;
  ...
  function project(): RuntimeFormSnapshot {
    ...
    for (const [field, kind] of Object.entries(definitions)) {
      ...
      fields[field] = {
        text: draft?.clear
          ? String(record(host.base)[field] ?? "")
          : draft
            ? draft.text
            : String(host.value?.[field] ?? ""),
    ...
    return {
      ...
      dirty:
        ...
        removalRef !== undefined,
      invalid: Object.values(fields).some((field) => field.invalid),
      busy,
      available,
      writable: available && host.writable === true,
      error,
      status,
      credential: {
        ...credential,
        removable: typeof credentials?.unset === "function",
        removalPending: removalRef !== undefined,
      },
    };
  }
  ...
  function startReplacement(
    ref: string,
  ): Promise<RuntimeWire<unknown>> | undefined {
    if (
      disposed ||
      !credentials ||
      credentialRef() !== ref ||
  ...
  async function save() {
    if (
      disposed ||
      busy ||
      (!drafts.size && replacement === undefined && removalRef === undefined)
    )
      return;
    const host = scope.getSnapshot();
    if (host.status !== "ready" || !host.writable) return;
    if (removalRef !== undefined && typeof credentials?.unset !== "function") {
      error = "credentialRemoveUnavailable";
      status = "idle";
      publish();
      return;
    }
    activeSave = {
      ...
      removal: removalRef !== undefined,
      credentialIntent,
      ref: removalRef ?? (search ? credentialRef() : ""),
    };
    for (const [field, draft] of drafts) {
      ...
      activeSave.fields.push({
        field,
        draft,
        write,
        required: !accepted(record(host.user), field, write),
      });
    }
    busy = true;
    error = undefined;
    status = "saving";
    let refresh = false;
    const acknowledgeCredential = () => {
      // Retire only the positively acknowledged intention, not newer drafts or
      // unrelated settings. Aggregate failure is not a credential rollback.
      if (activeSave?.credentialIntent === credentialIntent) {
        if (activeSave.replacement) replacement = undefined;
        if (activeSave.removal) removalRef = undefined;
      }
      refresh = true;
    };
    try {
      ...
      if (activeSave.removal && confirmed) {
        const ref = activeSave.ref;
        if (
          !credentials?.unset ||
          credentialRef() !== ref ||
          metadataRef !== ref ||
          ...
          if (disposed) return;
          if (response.result.ok) acknowledgeCredential();
          else confirmed = false;
        }
      }
      if (activeSave.replacement && confirmed) {
      ...
      confirmed =
        ...
        activeSave.fields.every(({ field, write }) =>
          accepted(record(final.user), field, write),
        );
      if (confirmed) {
  ...
  return {
    ...
    edit(field, text) {
      ...
      if (field === "apiKey") {
        credentialIntent++;
        replacement = text.trim() ? text : undefined;
        removalRef = undefined;
    ...
    reset(field) {
      ...
      if (field === "apiKey") {
        credentialIntent++;
        removalRef = credentialRef();
        replacement = undefined;
      } else drafts.set(field, { text: "", clear: true });
      error = undefined;
      status = "idle";
      publish();
    },
    ...
    dispose() {
      ...
      snapshot = {
        fields: Object.fromEntries(
          Object.keys(snapshot.fields).map((field) => [
            field,
            { text: "", overridden: false, invalid: false },
          ]),
        ),
        dirty: false,
        invalid: false,
