...
export function EditTaskModal(props: { t: (key: BoardKey) => string; task: TaskRecord; busy: boolean; requestError?: string; onClose: () => void; onSave: (body: Record<string, unknown>) => void }): ReactElement {
  ...
  return (
    <div className={k("modalBackdrop")} onMouseDown={(event) => { if (event.target === event.currentTarget) props.onClose(); }}>
      <form ref={dialogRef} className={k("modal")} role="dialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={validationError === undefined && props.requestError === undefined ? undefined : errorId} tabIndex={-1} noValidate onSubmit={submit}>
        <div className={k("modalHeader")}><h2 id={titleId}>{props.t("edit")}</h2><button type="button" className={k("iconButton")} aria-label={props.t("close")} onClick={props.onClose}><CloseIcon /></button></div>
