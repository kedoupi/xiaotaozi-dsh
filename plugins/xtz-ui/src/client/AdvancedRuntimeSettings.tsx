...
type RuntimeField =
  | keyof typeof RUNTIME_FIELDS.shell
  | keyof (typeof RUNTIME_FIELDS)["agent-loop"]
  | keyof (typeof RUNTIME_FIELDS)["web-search-deepseek"];
...
export function AdvancedRuntimeSettings({
  ...
  forms: Record<RuntimeNamespace, RuntimeForm>;
  ...
  const subscribeMirror = useCallback(
    (listener: () => void) => mirror.subscribe(listener),
    [mirror],
  );
  ...
  const states = {
    shell: useSyncExternalStore(
      forms.shell.subscribe,
      forms.shell.getSnapshot,
      forms.shell.getSnapshot,
    ),
    "agent-loop": useSyncExternalStore(
      forms["agent-loop"].subscribe,
      forms["agent-loop"].getSnapshot,
      forms["agent-loop"].getSnapshot,
    ),
    "web-search-deepseek": useSyncExternalStore(
      forms["web-search-deepseek"].subscribe,
      forms["web-search-deepseek"].getSnapshot,
      forms["web-search-deepseek"].getSnapshot,
    ),
  };
  useEffect(() => {
  ...
  const loading =
    host.status === "loading" || (host.status === "idle" && !host.error);
  ...
  return (
    <div className="dshH-advanced" aria-busy={loading || busy}>
      <h1>{t("nav")}</h1>
      <p className="dshH-advancedHint">{t("lede")}</p>
      <p
        className="dshH-advancedStatus"
        role="status"
        aria-live="polite"
        aria-atomic="true"
        data-error={failed}
      ...
      {namespaces.map((namespace) => {
        ...
        const replacing =
          ...
          (!!state.fields.apiKey.text.trim() || credential.removalPending);
        return (
          <section
            key={namespace}
          ...
          >
            ...
            {!state.available && !loading && (
              <p className="dshH-advancedHint">{t("unavailable")}</p>
            )}
            {state.available && !state.writable && (
              <p className="dshH-advancedHint">{t("readOnly")}</p>
            )}
            {(
              Object.entries(RUNTIME_FIELDS[namespace]) as [
                RuntimeField,
                "number" | "text",
              ][]
            ).map(([field, kind]) => {
              ...
              return (
                <div className="dshH-advancedRow" key={field}>
                  <label htmlFor={id}>{t(field)}</label>
                  <span className="dshH-advancedOverride">
                    {t(value.overridden ? "overridden" : "inherited")}
                  </span>
                  <div className="dshH-advancedControls">
                    <input
                      ...
                      value={value.text}
                      onChange={(event) =>
                        form.edit(field, event.currentTarget.value)
                      }
                    />
                    <button
                      type="button"
                      disabled={disabled}
                      onClick={() => form.reset(field)}
                    >
                      {t("reset")}
                    </button>
                  </div>
                  <p
                    id={`${id}-hint`}
                    className="dshH-advancedHint"
                    data-error={value.invalid}
                  >
            ...
            {namespace === "web-search-deepseek" && (
              <div className="dshH-advancedRow">
                <label htmlFor="web-search-deepseek-apiKey">
                  {t("apiKey")}
                </label>
                {!credential.error && (
                  <span className="dshH-advancedOverride">
                ...
                <input
                  ...
                  value={state.fields.apiKey.text}
                  onChange={(event) =>
                    form.edit("apiKey", event.currentTarget.value)
                  }
                />
                <button
                  type="button"
                  disabled={
                    disabled ||
                    !credential.removable ||
                    !credential.writable ||
                    credential.loading ||
                    credential.error
                  }
                  onClick={() => form.reset("apiKey")}
                >
                  {t("credentialRemove")}
                </button>
                {credential.removalPending && (
                  <p className="dshH-advancedHint">
                    {t("credentialRemovalPending")}
                  </p>
                )}
                {!credential.removable && (
                  <p className="dshH-advancedHint">
                    {t("credentialRemoveUnavailable")}
                  </p>
                )}
                <p
                  id="web-search-deepseek-apiKey-hint"
                  className="dshH-advancedHint"
                >
                  {t("apiKeyHint")}
                </p>
                <p
                  id="web-search-deepseek-apiKey-state"
                  className="dshH-advancedHint"
                  data-error={credential.error}
                >
                  {credential.error
                    ? t("credentialError")
                    : !credential.loading && !credential.writable
                ...
                disabled={
                  ...
                  (replacing &&
                    (!credential.writable ||
                      credential.loading ||
                      credential.error ||
                      (credential.removalPending && !credential.removable)))
                }
                onClick={() => {
                  void form.save();
                }}
