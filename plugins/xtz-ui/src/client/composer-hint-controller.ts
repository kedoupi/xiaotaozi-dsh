...
export function mutationTouchesComposer(record: MutationRecord): boolean {
  ...
  for (const node of Array.from(record.addedNodes).concat(
    Array.from(record.removedNodes),
  )) {
    if (isOwnComposerHintNode(node)) continue;
    if (!(node instanceof Element)) continue;
    if (
      node.matches(`${COMPOSER_CARD_SELECTOR}, ${COMPOSER_TEXTAREA_SELECTOR}`)
    )
      return true;
    if (
      node.querySelector(
        `${COMPOSER_CARD_SELECTOR}, ${COMPOSER_TEXTAREA_SELECTOR}`,
      ) != null
    )
      return true;
  }
...
function isComposerTextarea(
  target: EventTarget | null,
): target is HTMLTextAreaElement {
  return (
    target instanceof HTMLTextAreaElement &&
    target.closest(COMPOSER_CARD_SELECTOR) != null
  );
}
...
export function installComposerHint(doc: Document = document): () => void {
  ...
  const refresh = coalesce(() => {
    if (!disposed) syncComposerHint(doc);
  });
  ...
  return () => {
    ...
    for (const hint of Array.from(
      doc.querySelectorAll(`[${COMPOSER_HINT_ATTR}]`),
    ))
      hint.remove();
  };
}
