...
.dshXtzStickyButton { display: block; box-sizing: border-box; width: 100%; max-width: var(--dsh-chat-content-width, 748px); min-height: 36px; padding: 6px 12px; border: 0; border-radius: 12px; background: var(--dsw-specific-bubble); color: var(--dsw-alias-label-primary); font: inherit; font-size: 13px; line-height: 20px; text-align: left; pointer-events: auto; cursor: pointer; transition: background-color var(--xtz-dur-fast, 120ms) var(--xtz-ease-out), transform var(--xtz-dur-fast, 120ms) var(--xtz-ease-out); }
.dshXtzStickyButton:hover { background: color-mix(in srgb, var(--dsw-specific-bubble) 88%, var(--dsw-alias-label-primary)); }
.dshXtzStickyButton:active { transform: scale(0.99); }
...
@media (prefers-reduced-motion: reduce) { .dshXtzStickyBar { box-shadow: none; opacity: 1; transition: none; } .dshXtzStickyButton, .dshXtzStickyButton:active { transition: none; transform: none; } }
