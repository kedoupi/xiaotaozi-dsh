...
export const COMPOSER_EDITABLE_SELECTOR =
 "[contenteditable][data-phase], [contenteditable][data-input-scroll] [contenteditable]";
...
export function composerHintIsHeroCard(card: {
 closest(selectors: string): object | null;
}): boolean {
...
export function composerHintShouldWrite(
...
export function syncComposerHint(root: ParentNode): void {
 for (const card of root.querySelectorAll(COMPOSER_CARD_SELECTOR))
  applyCard(card);
}

function composerDraft(
 card: Element,
): { grow: Element; draft: string; placeholder: string } | undefined {
 const textarea = card.querySelector<HTMLTextAreaElement>(
  `:scope ${COMPOSER_TEXTAREA_SELECTOR}`,
 );
 if (textarea !== null && textarea.parentElement !== null) {
  return {
   grow: textarea.parentElement,
   draft: textarea.value,
   placeholder: textarea.placeholder,
  };
 }
 // RC1 contenteditable owns its own placeholder lifecycle, including absence
 // while typing. Only the legacy textarea needs our fallback.
 return undefined;
}

function applyCard(card: Element): void {
 if (card.querySelector("[data-composer-placeholder]") !== null) {
  for (const hint of card.querySelectorAll(`[${COMPOSER_HINT_ATTR}]`))
   hint.remove();
  return;
 }
 ...
 const existing = surface.grow.querySelector<HTMLElement>(
  `:scope > [${COMPOSER_HINT_ATTR}]`,
 );
 ...
 const hint =
  existing ??
  surface.grow.appendChild(surface.grow.ownerDocument.createElement("div"));
