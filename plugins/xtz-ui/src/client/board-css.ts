...
html[data-dsh-xtz-ui-board-active] [data-dsh-xtz-ui-board-view] {
  display: block;
}
...
html[data-dsh-xtz-ui-board-active] [data-pane='conversation'] > :not([data-dsh-xtz-ui-board-view]),
html[data-dsh-xtz-ui-board-active] [class*='centerCol'] > :not([data-dsh-xtz-ui-board-view]) {
...
.dshH-tb-cardSpinner {
  ...
  height: 10px;
  flex: none;
  border: 2px solid var(--dsw-xtz-status-warning-ink, #7a4a00);
...
.dshH-tb-primaryButton {
  ...
  font-size: 13px;
  font-weight: 600;
  color: var(--dsw-alias-button-info-label, #fff);
...
.dshH-tb-dangerButton {
  ...
  font-weight: 600;
  color: #fff;
  background: color-mix(in srgb, var(--dsw-alias-state-error-primary, #ec1313) 72%, black);
  border: none;
  border-radius: var(--xtz-radius-s, 8px);
  cursor: pointer;
  white-space: nowrap;
...
.dshH-tb-dangerButton:hover:not(:disabled) {
  background: color-mix(in srgb, var(--dsw-alias-state-error-primary, #ec1313) 64%, black);
}

.dshH-tb-dangerButton:active:not(:disabled) {
  background: color-mix(in srgb, var(--dsw-alias-state-error-primary, #ec1313) 56%, black);
}
...
.dshH-tb-iconButton {
  ...
  background: transparent;
  border: none;
  border-radius: var(--xtz-radius-s, 6px);
  color: var(--dsw-alias-label-secondary);
  cursor: pointer;
  font-size: 13px;
}
...
.dshH-tb-modalHeader h2 { margin: 0; font-size: 15px; line-height: 1.4; }
...
.dshH-tb-statusBadge[data-status='failed'] {
  color: var(--dsw-xtz-status-error-ink, #b42318);
  border-color: var(--dsw-xtz-status-error-ink, #b42318);
}
...
/* --- brand empty state (docs/brand.zh.md §4: empty states may show the
       face; §2.1: leaf is decoration only, never a status color) --- */
.dshH-tb-emptyBoard {
  flex: 1;
  min-height: 180px;
  display: grid;
...
.dshH-tb-emptyTray {
  ...
  place-items: center;
  border-radius: var(--xtz-radius-l, 16px);
  background: color-mix(in srgb, var(--dsw-xtz-brand-display, #fc9052) 14%, transparent);
}

.dshH-tb-emptyTray img {
  width: 64px;
  height: 64px;
  border-radius: 14px;
  display: block;
...
.dshH-tb-emptyLeaf {
  ...
  width: 12px;
  height: 12px;
  color: var(--dsw-xtz-brand-leaf, #98a92d);
}

.dshH-tb-emptyLeaf svg {
  display: block;
  width: 100%;
  height: 100%;
}
...
.dshH-tb-modal:focus-visible,
.dshH-tb-detail:focus-visible {
  outline: 2px solid var(--dsw-alias-state-business-primary);
  outline-offset: 2px;
}
...
.dshH-tb-schedulePreset,
.dshH-tb-scheduleToggle input {
  transition: background-color var(--xtz-dur-fast, 120ms) var(--xtz-ease-out, ease), color var(--xtz-dur-fast, 120ms) var(--xtz-ease-out, ease), border-color var(--xtz-dur-fast, 120ms) var(--xtz-ease-out, ease), outline-color var(--xtz-dur-fast, 120ms) var(--xtz-ease-out, ease), box-shadow var(--xtz-dur-fast, 120ms) var(--xtz-ease-out, ease), transform var(--xtz-dur-fast, 120ms) var(--xtz-ease-out, ease);
}
...
@media (pointer: coarse) {
  ...
  .dshH-tb-select,
  .dshH-tb-schedulePreset,
  .dshH-tb-scheduleToggle { min-height: 44px; }
}
