...
export const PRODUCT_IDENTITY_ROUTE = "/.well-known/xiaotaozi-dsh/identity/v1";
...
): { ok: true; config: XtzUiConfig; shipped: FeatureShipped; surfaces: ReturnType<typeof surfacesFor> } {
  return { ok: true, config, shipped, surfaces: surfacesFor(config, shipped) };
}

export function registerXtzUiSettingsRoute(
  webServer: WebServer,
  read: () => XtzUiConfig,
  write: (patch: Partial<XtzUiConfig>) => XtzUiConfig,
  shipped: FeatureShipped = FEATURE_SHIPPED,
): () => void {
  return webServer.register({
    kind: "exact",
    path: XTZ_UI_SETTINGS_ROUTE,
