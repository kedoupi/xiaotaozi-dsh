...
it("layers repository, current branch, and commit identity above metadata", () => {
  ...
  expect(gitGraphCss).toMatch(/\.dshH-gg-graphSubject\s*\{[^}]*font-weight:\s*600/su);
});
...
it("names the current branch and current commit in text and accessibility semantics", () => {
  ...
  expect((gitGraphEn as Record<string, string>).currentBranch).toBe("Current branch");
...
it("keeps long identifiers locally clipped and the 375px dialog contained", () => {
  expect(gitGraphCss).toMatch(/\.dshH-gg-graphLaneViewport\s*\{[^}]*overflow:\s*hidden/su);
  expect(gitGraphCss).toMatch(/\.dshH-gg-graphOid\s*\{[^}]*overflow:\s*hidden/su);
  expect(gitGraphCss).toMatch(/\.dshH-gg-graphRef\s*\{[^}]*text-overflow:\s*ellipsis/su);
  expect(gitGraphCss).toMatch(/@media \(max-width:\s*768px\)[\s\S]*\.dshH-gg-graphLaneViewport\s*\{[^}]*max-width:/u);
  expect(gitGraphCss).toMatch(/\.dshH-gg-dialog\s*\{[^}]*box-sizing:\s*border-box/su);
});

it("preserves dialog focus behavior and categorical lane rendering", () => {
  expect(source).toContain("useDialogFocus<HTMLDivElement>(props.onClose, closeRef)");
  ...
  expect(gitGraphCss).not.toMatch(/--dshH-gg-lane-[0-7]:\s*(?:#FC8940|#B94305|var\(--dsw-xtz-brand)/iu);
});
