...
describe("board ledger", () => {
  ...
  it("round-trips through DSH_HOME board.json", () => {
    const home = mkdtempSync(join(tmpdir(), "dsh-xtz-ui-board-"));
    dirs.push(home);
  ...
  it("adopts board.json from plugins/hello", () => {
    const home = mkdtempSync(join(tmpdir(), "dsh-xtz-ui-board-"));
    dirs.push(home);
    const env = { DSH_HOME: home };
    mkdirSync(join(home, "plugins", "hello"), { recursive: true, mode: 0o700 });
    writeFileSync(join(home, "plugins", "hello", "board.json"), `${JSON.stringify({ tasks: [{ id: "t1", title: "Legacy", prompt: "do", status: "todo", createdAt: 1, updatedAt: 1, executions: [] }] })}\n`);
    const loaded = loadBoard(env);
    expect(loaded).toHaveLength(1);
