...
Cordis patch：`id: xtz-ui`，`name: dsh-xtz-ui`。
...
- `GET|HEAD|POST /api/dsh-xtz-ui/settings`
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/dsh-xtz-ui/archives` | 列表；顺带 prune ghost ids |
| GET | `/api/dsh-xtz-ui/detail?sessionId=` | 预览；404 session not found |
| POST | `/api/dsh-xtz-ui/unarchive` | body `sessionIds` 或 `sessionId` |
...
| 方法 | 路径 |
| --- | --- |
| GET | `/api/dsh-xtz-ui/gg/status` |
| GET | `/api/dsh-xtz-ui/gg/branches` |
| GET | `/api/dsh-xtz-ui/gg/log?limit=`（1–400，默认 80） |
| POST | `/api/dsh-xtz-ui/gg/switch` body `{ sessionId, branch }` |

非仓库：status/branches/log 返回 `repo:false`。switch 在非仓库 400 `not a git repository`。

### 4.6 System prompt

`systemPrompt.section({ name:"xtz-ui:xiaotaozi", order:80, text })`。宣告关或无其他表面时返回空串。
...
`pnpm --filter dsh-xtz-ui test`（vitest）。主要用例：
