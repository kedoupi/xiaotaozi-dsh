# dsh-hello 产品需求文档（PRD）

- 产品：dsh-hello（小桃子 DSH 壳插件）
- 包名：`dsh-hello`
- 版本：0.8.0
- 状态：已实现（随小桃子 DSH 桌面种子 / 签名 pack 分发）
- 文档日期：2026-08-27
- 适用范围：本仓库 `plugins/hello` 当前源码。只描述已落地行为；规划项单独标注。
...
4. 给 Desktop / `xtz doctor` 提供本机 loopback 身份探测：`/.well-known/xiaotaozi-dsh/identity/v1`。

右侧文件 / Git / 终端工作台不在本插件，而在 `dsh-sidebar`。模型、记忆、IM、上下文、agent-teams 仍在各自插件。

## 2. 用户与场景

| 角色 | 场景 |
| --- | --- |
| 桌面用户 | 安装小桃子DSH.app 后看到小桃子品牌、欢迎弹框；在「设置 → 小桃子」开关归档 / 看板 / Git 图谱。 |
| 插件作者 | 沙箱 `.dsh-home` :3081 用 `link-plugin hello` 调试壳层。 |
| 双持用户 | Desktop 写正式 `~/.dsh`；`xtz status` / `xtz doctor` 只读探测 identity 路由。 |
...
**FR-05 欢迎队列**  
`src/notices.ts` 队列；当前仅 `xiaotaozi-welcome`。dismissed ids 存在本 origin `localStorage` 键 `dsh-hello.dismissed`。每个 id 只出现一次。
...
**FR-07 设置持久化**  
`$DSH_HOME/plugins/hello/settings.json`，目录 0700、文件 0600、tmp+rename。未知键丢弃。
...
**FR-13 向 Agent 宣告**  
仅当 `announceToAgent=true` 且至少还有一个表面打开。写入 systemPrompt section `hello:xiaotaozi` order 80。
...
设置页 GET `/api/dsh-hello/settings` → 拨动开关 POST patch → Host 写 settings.json → remount 对应路由/调度 → 客户端订阅刷新入口。
...
5. 默认 archive/board/gitGraph 开、announce 关。关 archive 后设置无归档页、无 `/api/dsh-hello/archives`。
...
10. `pnpm --filter dsh-hello test` 通过。
