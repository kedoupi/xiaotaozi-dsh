...
import {
  HELLO_IDENTITY,
  HELLO_IDENTITY_ROUTE,
  registerHelloIdentityRoute,
  settingsPayload,
  type WebServer,
...
describe("Desktop identity route", () => {
  it("is versioned, loopback-only, GET-only, and returns fixed readiness identity", async () => {
    ...
    expect(route.path).toBe(HELLO_IDENTITY_ROUTE);

    const first = await requestIdentity(route, "GET");
    const second = await requestIdentity(route, "GET", "::ffff:127.0.0.1");
    expect(first.status).toBe(200);
    expect(first.body).toEqual(HELLO_IDENTITY);
    expect(second.body).toEqual(HELLO_IDENTITY);
  ...
  it("echoes only a Desktop-generated instance token with the strict shape", async () => {
    ...
    expect((await requestIdentity(identityRoute("not-a-token"), "GET")).body).toEqual(HELLO_IDENTITY);
  });
});
