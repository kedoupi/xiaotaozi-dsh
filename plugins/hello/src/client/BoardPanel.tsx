...
import { HELLO_BOARD_NAMESPACE, HELLO_BOARD_PREFIX } from "../names.ts";
...
export function BoardPanel(props: { ctx: ClientContext; panel: PanelOpen }): ReactElement {
  const t = props.ctx.locale.bind(HELLO_BOARD_NAMESPACE) as (key: BoardKey) => string;
  ...
  const load = useCallback(async (): Promise<void> => {
    try {
      const payload = await fetchJson(HELLO_BOARD_PREFIX);
  ...
  const post = async (path: string, body: unknown): Promise<void> => {
    ...
    setBusy(true);
    try {
      const payload = await fetchJson(`${HELLO_BOARD_PREFIX}${path}`, {
        method: "POST",
  ...
  return (
    <div className={k("board")} data-dsh-plugin="hello-board">
