...
import { HELLO_GG_PREFIX, HELLO_GIT_GRAPH_NAMESPACE } from "../names.ts";
...
function GraphDialog(props: {
  ...
  const load = useCallback((limit: number): void => {
    ...
    setLoading(true);
    void fetchJson(`${HELLO_GG_PREFIX}/log?${qs(props.sessionId, { limit: String(limit) })}`).then((payload) => {
...
export function GitGraphChip(props: {
  ...
  const t = props.ctx.locale.bind(HELLO_GIT_GRAPH_NAMESPACE) as (key: GitGraphKey) => string;
  ...
  const loadStatus = useCallback(async (): Promise<void> => {
    ...
    try {
      const payload = await fetchJson(`${HELLO_GG_PREFIX}/status?${qs(sessionId)}`) as StatusPayload;
      setStatus(payload);
      setError(undefined);
    } catch {
  ...
  const runSwitch = async (branch: string): Promise<void> => {
    try {
      await fetchJson(`${HELLO_GG_PREFIX}/switch`, {
        method: "POST",
    ...
    <span className="dshH-gg-chipWrap">
      <button
        ...
        onClick={() => {
          setOpen((value) => !value);
          if (!open) {
            void fetchJson(`${HELLO_GG_PREFIX}/branches?${qs(sessionId)}`).then((payload) => {
