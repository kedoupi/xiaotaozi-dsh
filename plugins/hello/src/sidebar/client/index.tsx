...
import { getSettingsSnapshot, loadSettingsLive, subscribeSettings } from '../../client/settings-live.ts'
  ...
  try {
    ...
    ctx.effect(() => {
      ...
      const sync = async (): Promise<void> => {
        ...
        if (disposed) return
        const helloOff = !getSettingsSnapshot().surfaces.includes('workbench')
        const off = suspended || helloOff
        sidebarStore.setSuspended(off)
        if (off) unmount()
        else mount()
      }
      void sync()
      void loadSettingsLive().then(() => { void sync() }).catch(() => { void sync() })
      const offHello = subscribeSettings(() => { void sync() })
    ...
    ctx.effect(
      () => {
        try {
          // External http(s) links only enter the sidebar when a registered
          // tab claims the URL (`urlTarget`). There is no built-in browser
          // tab; unclaimed links stay with the system browser.
          ...
          return registerLinkInterception({
            takeoverEnabled: (url) => {
              ...
              return urlTargetOf(url) !== undefined
            },
            openInSidebar: (url) => {
              let title: string | undefined
              try { title = new URL(url).hostname } catch { /* keep the default title */ }
              const type = urlTargetOf(new URL(url))
