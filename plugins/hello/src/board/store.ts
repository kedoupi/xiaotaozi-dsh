...
export function helloBoardPath(env: NodeJS.ProcessEnv = process.env): string {
  return join(dshHome(env), "plugins", "hello", "board.json");
}
...
export function loadBoard(env: NodeJS.ProcessEnv = process.env): TaskRecord[] {
  const parsed = readJsonFile(helloBoardPath(env));
...
export function saveBoard(tasks: readonly TaskRecord[], env: NodeJS.ProcessEnv = process.env): void {
  writeJsonFile(helloBoardPath(env), { tasks });
}
