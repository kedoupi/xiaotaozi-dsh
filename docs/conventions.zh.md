...
开放命令：帮助/版本、直接运行 `xtz` / `start` / `stop` / `restart` / `open` / `status` / `doctor` / `config path`。`web` 是 start 的别名。`xtz` 是钉死版本的 dsh 外壳，不是插件管理器。第一次 `xtz start` 种正式 web 和 `plugins/` 下每一个自研插件。CLI 产品升级后，服务已停止的 `start` / `restart` 会把全部默认插件作为一个可回滚的 profile 事务同步到精确产品规格；服务运行时，`start` 只提示执行 `xtz restart`，绝不热改 profile。同步失败会恢复原 profile，且不启动 Web。额外（第三方）插件保留在 profile 中，通过应用内市场安装。已停止的 `start` / `restart`（沙箱或正式）若发现额外插件的 Host 入口不存在（例如 Git 安装没有 `lib/`），或 Client `inject` 在等 `uiConversation`（DSH 0.1.2 会卡住整页 Web 启动），只把它从 `dsh.profile.bundles` 拿掉，保留 dependency，打印被隔离的插件，然后照常启动 Web。自研插件和 DSH 核心 bundle 不会走这条隔离。应用内市场分别验证依赖、bundle 成员和入口文件；这不代表运行时激活成功。不完整或未验证的变更仍报错；因 pinned 插件命令会协调整个 profile，不会尝试不安全的自动回滚。先检查保留状态，再从「发现插件」修复或在「已安装」明确移除。市场不会依据子进程输出提升构建脚本授权，也不自动重试被阻止的构建；现有拒绝和待定授权保持不变。`status` 和 `doctor` 只接受 `/.well-known/xiaotaozi-dsh/identity/v1` 的精确 v1 响应；其他 HTTP 响应只能证明端口被占用。Identity `ready: true` 只证明 Host 健康，不证明 SPA 已加载。

DSH 0.1.2 的 `dsh web` 需要 BrowserAuth：`GET /?token=<launchToken>` 签发 cookie 并 303 到 `/`。没有 cookie 时直接打开 `/` 是 401。启动令牌是进程随机数（同一进程内可重复使用），不是 identity 的 `instanceToken`。`xtz start` 捕获完整的 loopback `dsh web:` 公告，写入 `$DSH_HOME/xiaotaozi-xtz-web.auth`（权限 0600），打印并打开它。`open` 要求认证文件与已验证归属的进程代次、PID、host 和端口匹配。新启动在拉起进程前清除孤立的旧认证文件；restart 捕获新进程令牌。不要把 token 贴进日志或 issue；认证失败诊断不打印令牌。
