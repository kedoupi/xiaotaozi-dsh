...
4. 用当前 `main` 更新、重跑必需门禁、按 [模板](../.github/pull_request_template.md) 开 PR。完成 [CR](#代码审查与人工验收)，记录审查 base/head SHA；后续改动要补审差异。
5. 审查阻塞问题解决，且审过的 head 的必跑 GitHub CI 绿了再合。协调相关 PR，不要在待验收改动上连续叠加依赖改动。
6. 确认审过的改动已包含在 `origin/main` 中（记录最终 merge/squash/rebase 提交及其 PR 映射）。给 PR 标记 `qa:pending`，已有更新验收记录时不要覆盖。
...
9. 在合并后的 `main` 上做已授权的集成检查，记录实际运行 SHA 和结果，将受影响旅程的步骤/预期交给用户做 [人工验收](#代码审查与人工验收)。Agent 检查不把 PR 标为通过；无法执行的检查明确列为阻塞或未验证项。
...
11. 先在 PR 留下待验/失败/通过的交接记录与 [学习总结](#每次交接的学习总结)，再删除已合并的本地/远端主题分支和干净的 task worktree。待人工验收状态留在 PR 中，不因清理消失；清理不能称为任务验收通过。绝不强制清理；脏 worktree 必须保留并报告，直到它的负责人提交或移走工作。
...
规范：[conventions.zh.md](conventions.zh.md#代码审查人工验收与学习)。本节补充上面的循环，不授权原本未获准的合入、服务操作、浏览器副作用或权限变更。

### 合入前：CR

1. 读取需求/issue 和相关历史 PR 的经验。填写 [PR 模板](../.github/pull_request_template.md)，包括范围、检查与拟定的人工验收步骤。
2. 固定 base/head SHA，对照 diff 审真实调用链。优先检查正确性、状态/账号/会话隔离、取消/重试、回滚、安全以及跨 PR 集成。当前会话直接进行单独的审查轮次；属于自审就明确标注，不冒充独立批准。
3. 每个问题记录严重度、文件:行号、触发条件、影响、证据和处理结论。解决阻塞项，明确剩余风险与未验路径。测试全绿不替代 CR。
4. 合入前再次检查当前 `main` 和 PR head。任一变化就补审集成差异，重跑受影响的门禁，证据绑定新的审查 head。只有必需的合入前 QA 才显式有界移交 3081，结束后还给 hub。常规人工验收仍在合入后的 `main` 上做。

### 合入后：交接与人工决定

1. 交付 Agent 负责待验交接，现有 hub 负责人负责更新/重启/检查。先协调，不让多个 Agent 各自重启 3081。正式 3080 不动。
2. 先读 PR 最新评论和标签，没有更新决定才设 `qa:pending`。从已核验的 hub 启动/重启记录确认运行 SHA，不只看当前 Git HEAD；记录检查结果、阻塞和简短的 **入口 → 操作 → 预期** 清单。纯文档改动写明文档 SHA 和审阅步骤，不虚构运行旅程。
3. 用户可以告诉任意 Agent「main sandbox 上 PR #225 和 #226 人工验收通过」，该 Agent 可以在明确点名的 PR 代记。PR/范围不清就先确认，不推断未点名的 PR 也通过。用关联交接记录确定受测版本；测试期间服务变了，就按已知 SHA 分开记录，或请求重新验证。不要拿今天的 HEAD 猜过去运行的 SHA。
4. 写入前立即重读最新评论/标签。按下面格式追加验收评论，再仅替换已有 `qa:*` 状态标签，保留无关标签。部分通过按规范保持待验/失败。写完回读评论和标签。如果另一 Agent 已记录冲突决定，先和用户核对，不覆盖。评论或标签操作失败就说明同步未完成，不能声称已登记。缺少 SHA/范围证据时可以先记录人工反馈与缺失项，但不得设为 `qa:passed`。
5. 人工不通过就设 `qa:failed`，关联已有 issue 或带复现证据提单，有修复/回退 PR 后补上链接。只在独立 worktree 修，通过审查与绿 CI 合入。修复合入不清除失败标签；请求人工复验，在每条明确复验通过的 PR 上追加新决定。
6. PR 即使已关闭，也在其中追加人工反馈和学习。不为更新验收另造一次合入后报告提交。每个 Agent 在交接/宣布完成前都要重读记录；待验只能称为「已交付，待人工验收」，不能称为「全部完成」。
...
人工验收：通过 | 不通过 | 部分通过
PR：#...
环境 / 实际受测 SHA：main sandbox / ...（纯文档：文档 SHA）
范围 / 结果 / 剩余项：...
人工确认者 / 确认时间（含时区）：...
依据：人工 PR 评论链接，或当前 Agent 注明来源代记的用户明确确认
关联 issue / 修复 PR：...（如有）
学习：认识 → 证据 → 下次行动（或无新增学习）
```

仓库初始化：获得维护者授权后，按规范创建缺失的三个标签；不改 CI，不加自动批准机器人。标签和评论由人/Agent 维护，不会自动强制。缺标签不等于通过。查询要包含**已关闭的 merged PR**：`is:pr is:merged label:"qa:pending"` 或 `is:pr is:merged label:"qa:failed"`。模板合入不会给已有 PR 自动补记录；启用时明确约定历史批次，只初始化其中尚无验收决定的 PR。

### 每次交接的学习总结

1. 在 PR 或现有任务记录中写 1–3 条具体的 **认识 → 证据 → 下次行动**；两者都没有就写在最终回复里。阻塞、待验收时也写，没有新发现就说「无新增学习」，不凑心得。
2. 人工验收后追加真实使用确认或推翻了什么，不把早期记录改写成当时已经有这些证据。
3. 下次相关任务先读这些经验。可复现失败在修复 PR 中补回归测试；经验证的可复用步骤更新现有流程，长期规则变更先确认。一次性观察留在 PR，不新建知识系统，不强制报告型提交。

### 每日集成审查（按需执行）

固定时区、已合入 PR 批次和 main base/head SHA。逐条核对 PR 意图，再审最终 main 快照里的跨 PR 干扰、修复覆盖、接口漂移和测试缺口。在现有审查/任务记录中报告覆盖范围、有证据的问题和未验旅程，并做学习总结。它补充合入前 CR，不替代人工验收，也不授予 `qa:passed`。不要仅因存在本步骤就新开定时任务或监控服务。
...
1. 在已运行的 Web 界面，从 **新会话** 下方的左侧工具条打开 **插件中心**。首次欢迎确认会直接打开模型；Escape 只关掉欢迎卡。
2. 第一方配置走 **插件中心 → 已安装 → 模型/IM 机器人/小桃子功能/侧边工作台**。企业微信办公仍在 IM 机器人内的企业微信机器人卡片上。归档也可从会话 ⋯ 菜单打开。
3. 在 **发现插件** 搜索精选目录并安装。打开已安装第三方详情，确认后移除；移除不承诺删除已保存数据或凭据。
4. 运行参数走 **设置 → 高级**；故障诊断用 `xtz doctor`，不是技术插件清单设置页。设置第一次停在官方模型页时，会一次性转到插件中心 → 模型。
