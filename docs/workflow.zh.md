...
3. 立刻删掉模板里的 `greet` 样例，换成这个插件真正要做的事。纯逻辑放在不依赖 Cordis 的文件里，测试只测那些文件。不要把模型 / 记忆 / IM / 企业微信办公 / 市场 / 上下文 / agent-teams / 右侧文件-Git-终端面板塞进 `hello`；那个插件是壳加上归档、任务看板和 Git 图谱。右侧面板是 `plugins/sidebar`。
...
1. 让 `pnpm dev` 一直跑。沙箱 trace 保持开（每个插件 host 都是 `DSH_PLUGIN_TRACE=1`；正式包 / 小桃子DSH.app 不打）。
