...
- Exactly two homes:
  1. Desktop / official: `~/.dsh`, port **3080**. Bundled Node+dsh. Plugin updates are silent packs from `https://s.xiaotaozi.cc/dsh/packs/` only (existing TCB COS prefix; never GitHub/npm/`link:` / `dsh.xiaotaozi.cc`). Index is an Ed25519 envelope and clients verify it. COS put is not a publish: `pnpm publish-pack` must `PurgeUrlsCache` and wait for the live index. `pnpm check-home` only diagnoses; it never edits profiles.
