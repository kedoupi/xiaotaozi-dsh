...
<p align="center"><b>小桃子 DSH：DeepSeek Harness 插件和 Win/Mac 桌面客户端。一个插件包做一件事。</b></p>
...
[DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) 从 Git 或 npm 装插件。本仓库是 **xiaotaozi-dsh**：`plugins/` 下一个目录就是一个 npm 包（`dsh-<slug>`）。Win/Mac 客户端在 [`apps/desktop/`](apps/desktop/)（Tauri，不是 pnpm workspace 成员）。根目录不是插件，不要对它执行 `dsh plugin add`。

出了问题，或还缺某个插件？[提个 issue](https://github.com/kedoupi/xiaotaozi-dsh/issues)。

## 特性

- **一个包做一件事。** 模型、记忆、IM 机器人、欢迎弹框各自安装。Git 安装一律是 `github:kedoupi/xiaotaozi-dsh#path:plugins/<slug>`。
- **界面中文，默认英文文档。** 小桃子插件给用户看的文案是中文。对外 README 默认英文，中文在 `README.zh.md`。
- **两套家目录。** 小桃子DSH 的 Tauri 客户端和官网 `dsh web` 共用 `~/.dsh`（端口 3080）。安装包内置 Node + dsh。调插件用 `.dsh-home`（`pnpm dev`，3081）。不要混。
...
scripts/            new / link-plugin / check-manifest / doctor / 沙箱启动
docs/               规范 + 流程
.dsh-home/          gitignore 掉的沙箱 Harness 家目录（端口 3081）
```

根包不要声明 `dsh.bundle` 或 `dsh.profile`。
dsh RC、Node、Python、pnpm、桌面应用版本只有一个机器可读规范源：[`versions.json`](versions.json)。各清单仍写普通字面值，不让 `package.json` 动态引用 JSON；`pnpm check` 负责拒绝漂移。

| | 官网 / 小白桌面端 | 沙箱 |
| :-- | :-- | :-- |
| 家目录 | `~/.dsh` | `<仓库>/.dsh-home`（gitignore） |
| 命令 | 小桃子DSH.app 或 `dsh web` | `pnpm dev` |
| 端口 | **3080** | **3081** |
