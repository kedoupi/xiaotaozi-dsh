...
1. 准备官方 Harness home：`~/.dsh/profiles/web`（第一次，或你把该 profile 挪开之后）。
2. 种上全部六个自研插件 —— 模型、IM 机器人、企业微信办公、小桃子界面、侧边工作台、插件中心。升级 CLI 之后，**服务已停止**的 `xtz start` 会把这些默认插件对齐到新快照；见[常见问题](/zh/guide/faq)。
3. 在 `127.0.0.1:3080` 后台启动官方 `dsh web` 服务。
4. 打印一次性认证 URL（`/?token=…`）并打开浏览器。没有这张 cookie 时打开 `/` 会 401。
...
- 接入模型厂商：打开 **插件中心 → 已安装 → 模型**。见[插件介绍](/zh/guide/plugins)。
- 在 IM 里和 Agent 聊天：**插件中心 → 已安装 → IM 机器人**。
- 安装更多插件：**插件中心 → 发现插件**。见[插件市场](/zh/guide/market)。
