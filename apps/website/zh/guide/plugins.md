...
| 插件 | 位置 | 作用 |
| :-- | :-- | :-- |
| `dsh-providers` | 设置 → **模型** | 会员登录和 API Key 一页搞定；聊天里只列出你勾选的模型 |
| `dsh-im` | 侧边栏 → **IM bots** | 九个聊天渠道，外加实验性的 AI Office 连接器 |
| `dsh-wecom-office` | 设置 → **企业微信办公** | 企微日历、文档、会议、通讯录、表格、待办和微盘 |
...
让模型通过官方 `wecom-cli` 使用企微日历、在线文档、会议、通讯录、表格、待办和微盘。在 设置 → **企业微信办公** 里配置。
